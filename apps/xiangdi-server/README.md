...
| `pages_snapshot` | `{ pages: string[] }` | 写操作完成后实时推送当前 pages |
...
| `done` | `{ pages: string[], threadId }` | Agent 执行完成，携带最终 pages |
...
| `pages` | `string[]` | 否 | 当前最新的 pages 数据（由 banyan 后端从 MongoDB 读取后传入） |
