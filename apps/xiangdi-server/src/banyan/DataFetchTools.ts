...
const GET_SCHEMA_DEFINITION: ToolDefinition = {
    name: 'app_get_schema',
    description: '获取当前应用的数据库表结构定义（CollectionSchema）。返回所有集合及其字段信息。在需要了解数据模型、创建/修改数据表、或设计与数据相关的功能前调用此工具。',
    input_schema: {
        type: 'object',
        properties: {},
        required: [],
...
const GET_CLOUD_FUNCTIONS_DEFINITION: ToolDefinition = {
    ...
    description: '获取当前应用的所有云函数列表（摘要信息：functionId、名称、描述）。不包含完整的 FlowSchema，如需查看某个云函数的详细编排逻辑，请使用 app_get_cloud_function_detail 工具。',
    input_schema: {
        type: 'object',
        properties: {},
        required: [],
...
const GET_CLOUD_FUNCTION_DETAIL_DEFINITION: ToolDefinition = {
    ...
    description: '获取单个云函数的完整详情，包括 FlowSchema（nodes + edges 定义）。在需要理解或修改某个特定云函数的编排逻辑时调用。',
    input_schema: {
        type: 'object',
        properties: {
            functionId: {
...
export function registerDataFetchTools(
    ...
    registry.register(
        GET_CLOUD_FUNCTION_DETAIL_DEFINITION,
        async (input: { functionId: string }) => {
