...
export class BanyanClient {
  ...
  async getUIDefinition(appId: string): Promise<string> {
    const resp = await this.get<{ success: boolean; data: { uiJSON: string } }>(
      `/internal/apps/${appId}/ui-definition`,
    )
  ...
  async getSchema(appId: string): Promise<SchemaCollectionInfo[]> {
    const resp = await this.get<{ success: boolean; data: { collections: SchemaCollectionInfo[] } }>(
      `/internal/apps/${appId}/schema`,
    )
  ...
  async getCloudFunctions(appId: string): Promise<CloudFunctionInfo[]> {
    const resp = await this.get<{ success: boolean; data: { functions: CloudFunctionInfo[] } }>(
      `/internal/apps/${appId}/cloud-functions`,
    )
  ...
  async getCloudFunction(appId: string, functionId: string): Promise<CloudFunctionInfo | null> {
    const resp = await this.get<{ success: boolean; data: { function: CloudFunctionInfo } }>(
      `/internal/apps/${appId}/cloud-functions/${functionId}`,
    )
  ...
  async searchMaterials(
    ...
    { material_id: string; name: string; description?: string; tags?: string[]; parameterNames?: string[] }[]
  > {
    ...
    }>(`/api/materials/search?${params.toString()}`)
      ...
      const req = transport.request(options, (res) => {
        ...
        res.on('error', (err) => {
          ...
          reject(
            new ServiceUnavailableError(
              'BanyanBackend',
              `Response error for GET ${path}: ${err.message}`,
              err,
            ),
          )
        })
