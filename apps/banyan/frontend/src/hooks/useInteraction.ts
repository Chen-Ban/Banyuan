...
import { InteractionStateMachine, resolveActivationTarget } from '@/canvas/interaction'
import type {
  InteractionState,
  InteractionOutput,
  InteractionDelegate,
...
/** Flow 场景能力集 */
const FLOW_CAPABILITIES: readonly InteractionCapability[] = ['pan', 'move', 'connect', 'drop'] as const
...
export function useInteraction({
  ...
  const machine = useMemo(() => {
    ...
    const delegate: InteractionDelegate = {
      ...
      // ── 缩放 ──
      resize(view: View, fixedPoint: Point3, dynamicPoint: Point3, vector: Vector3, proportional: boolean) {
      ...
      // ── 文本选择 ──
      textInteract(view: View, point: Point3, bufferCtx: IDrawingContext) {
      ...
      finishConnect(edge: EdgeView, point: Point3) {
        ...
        if (mode === 'flow') {
          ...
          for (const view of children) {
            ...
            if (hit.maxConnections != null && hit.maxConnections !== Infinity) {
              const existingCount = children.filter(
                (v) => isEdgeView(v) && (v.toPortId === hit.id || v.fromPortId === hit.id),
              ).length
  ...
  useEffect(() => {
        ...
        for (const view of [...actived]) {
          if (isEdgeView(view)) {
            ...
            const relatedEdges = children.filter(
              (v): v is EdgeView =>
                isEdgeView(v) &&
                !!(v.fromPortId?.startsWith(nodeId + '_') || v.toPortId?.startsWith(nodeId + '_')),
            )
    ...
    const onInputEvent = (e: Event) => {
      const selectedView = actions?.view.getSelectedView(ViewType.TEXTVIEW) ?? null
    ...
    const onInputKeyDown = (e: Event) => {
      ...
      const selectedView = actions?.view.getSelectedView(ViewType.TEXTVIEW) ?? null
      if (!selectedView || !selectedView.selection.isSelection || !inputEl) return
        ...
        case 'Tab':
          ke.preventDefault()
          {
            ...
            if (editableViews.length > 0) {
              const currentIndex = editableViews.findIndex((v) => v === selectedView)
              const nextIndex = (ke.shiftKey ? currentIndex - 1 : currentIndex + 1) % editableViews.length
              ...
              if (bounds && canvas) {
                ...
                if (layoutBounds) {
                  const scene = getScene()
                  if (!scene) return
                  const screenPos = worldToScreen(worldBottomLeft.x, worldBottomLeft.y, scene, canvas)
