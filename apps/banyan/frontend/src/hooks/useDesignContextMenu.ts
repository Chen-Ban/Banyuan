...
function createViewContextMenuItems(
  ...
  const canGroup = activedViews.length >= 2 && activedViews.every((v) => v.parent === activedViews[0].parent)
...
export function useDesignContextMenu(actions: IBanvasActions | null): UseDesignContextMenuResult {
