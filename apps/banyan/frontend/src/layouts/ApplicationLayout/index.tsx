...
// ── 机型预设（分组） ────────────────────────────────────────────────────────────
interface DevicePreset { key: string; label: string; width: number; height: number }
interface DeviceGroup { group: string; icon: React.ReactNode; items: DevicePreset[] }
...
// 扁平化用于查找当前选中项
const ALL_DEVICE_PRESETS = DEVICE_GROUPS.flatMap((g) => g.items)
