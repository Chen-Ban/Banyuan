...
export class InteractionStateMachine {
  ...
  private onPointerDown(input: PointerDownInput): InteractionOutput {
    ...
    if (
      this.hasCapability("pan") &&
      (button === 1 || this._delegate.isSpaceHeld())
    ) {
    ...
    switch (action) {
      ...
      case Action.TEXT_SELECTION:
        if (!this.hasCapability("text-selection"))
          return { stateChanged: false };
        this._state = {
          mode: "text-selecting",
          indicateView: view,
  ...
  private onPointerUp(input: PointerUpInput): InteractionOutput {
    ...
    switch (prevMode) {
      case "panning": {
        ...
        const cursor = this._delegate.isSpaceHeld()
          ? Cursor.Grab
          : Cursor.Default;
        this._state = { mode: "idle" };
  ...
  private handleTextSelecting(worldPoint: Point3): InteractionOutput {
    ...
    const { content } = this._delegate.textInteract(
      view,
      worldPoint,
      bufferCtx,
    );
