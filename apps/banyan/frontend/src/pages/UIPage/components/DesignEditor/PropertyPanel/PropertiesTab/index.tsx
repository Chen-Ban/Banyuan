    ...
    if (cornersExpanded) {
      ...
      const r =
        typeof style.borderRadius === 'number'
          ? style.borderRadius
          : ((style.borderRadius as number[])?.[0] ?? 0)
      ...
      <section className={styles.section}>
        ...
        <div className={styles.transformGrid}>
          ...
          <NumberInput
            label="缩放宽"
            value={scaleX}
            onChange={(v) => actions.view.setProperty('scaleX', v)}
            onFocus={onFocus}
            onBlur={onBlur}
            step={0.1}
            precision={2}
          ...
          <NumberInput
            label="缩放高"
            value={scaleY}
            onChange={(v) => actions.view.setProperty('scaleY', v)}
            onFocus={onFocus}
            onBlur={onBlur}
            step={0.1}
            precision={2}
      ...
      <section className={styles.section}>
        ...
        <div className={styles.infoRow}>
          ...
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            />
            <span
              style={{ fontSize: 11, color: 'var(--color-text-tertiary)', minWidth: 32, textAlign: 'right' }}
            >
        ...
        {cornersExpanded && (
          <div className={styles.transformGrid}>
            <NumberInput
              label="左上"
              value={(style.borderRadius as number[])?.[0] ?? 0}
              onChange={(v) => {
                const r = [...((style.borderRadius as number[]) ?? [0, 0, 0, 0])]
                r[0] = v
                setStyle('borderRadius', r)
              }}
              onFocus={onFocus}
            ...
            <NumberInput
              label="右上"
              value={(style.borderRadius as number[])?.[1] ?? 0}
              onChange={(v) => {
                const r = [...((style.borderRadius as number[]) ?? [0, 0, 0, 0])]
                r[1] = v
                setStyle('borderRadius', r)
            ...
            <NumberInput
              label="右下"
              value={(style.borderRadius as number[])?.[2] ?? 0}
              onChange={(v) => {
                const r = [...((style.borderRadius as number[]) ?? [0, 0, 0, 0])]
                r[2] = v
                setStyle('borderRadius', r)
            ...
            <NumberInput
              label="左下"
              value={(style.borderRadius as number[])?.[3] ?? 0}
              onChange={(v) => {
                const r = [...((style.borderRadius as number[]) ?? [0, 0, 0, 0])]
                r[3] = v
                setStyle('borderRadius', r)
        ...
        <div className={styles.switchRow}>
          ...
          <Switch size="small" checked={clipContent} onChange={(v) => setStyle('clipContent', v)} />
        </div>
      </section>
