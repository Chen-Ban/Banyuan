...
export const EventsTab: React.FC<EventsTabProps> = (props) => {
    ...
    const eventCategories: { label: string; keys: { key: keyof IViewEvents; label: string }[] }[] = [
        { label: '点击', keys: boundEventKeys.filter(e => ['onClick', 'onDoubleClick', 'onContextMenu'].includes(e.key)) },
        { label: '鼠标', keys: boundEventKeys.filter(e => ['onMouseEnter', 'onMouseLeave', 'onMouseMove', 'onMouseDown', 'onMouseUp'].includes(e.key)) },
        { label: '拖拽', keys: boundEventKeys.filter(e => ['onDragStart', 'onDrag', 'onDragEnd'].includes(e.key)) },
        { label: '焦点', keys: boundEventKeys.filter(e => ['onFocus', 'onBlur'].includes(e.key)) },
    ].filter(cat => cat.keys.length > 0)

    const handleConfirmAddEvent = () => {
        if (!newEventKey) return
        actions.view.setViewEvent(selectedViewId, newEventKey, { version: FLOW_SCHEMA_VERSION, entry: '', nodes: {} })
        setNewEventKey('')
