...
const DefaultValueInput: React.FC<DefaultValueInputProps> = ({ schema, onChange }) => {
  ...
  if (!focused && prevSchema.current !== schema) {
    ...
    const next = schema.default === undefined || schema.default === null ? '' : String(schema.default)
...
const FieldRow: React.FC<FieldRowProps> = ({
  ...
  return (
    <div style={{ marginBottom: 2, paddingLeft: depth * 10 }}>
      <div className={styles.fieldRow}>
        ...
        <div className={styles.fieldKeyCell}>
          ...
          />
          {keyError && <div className={styles.fieldKeyError}>{keyError}</div>}
        </div>
        ...
        {/* 删除 */}
        <button className={styles.deleteBtn} onClick={() => onDelete(fieldKey)} title="删除字段">
          ×
        </button>
      </div>
...
export const FieldSchemaMapEditor: React.FC<FieldSchemaMapEditorProps> = ({
  ...
  return (
    <section className={depth > 0 ? styles.sectionNested : styles.section}>
      {title && <div className={styles.sectionHeader}>{title}</div>}

      {entries.length === 0 && depth === 0 && <div className={styles.emptyFields}>暂无字段</div>}
