...
function getDecorationConfig(type: DeviceType): DecorationConfig {
  switch (type) {
    case 'iphone':
      return {
        topBarHeight: 40, bottomBarHeight: 20, borderRadius: 24,
        showTopBar: true, topBarStyle: 'ios-status',
        showDynamicIsland: true, showPunchHole: false, showNotch: false,
        showHomeIndicator: true, homeIndicatorStyle: 'ios',
        showCrease: false, showUDC: false,
      };
    case 'android':
      return {
        topBarHeight: 34, bottomBarHeight: 18, borderRadius: 18,
        showTopBar: true, topBarStyle: 'android-status',
        showDynamicIsland: false, showPunchHole: true, showNotch: false,
        showHomeIndicator: true, homeIndicatorStyle: 'android-pill',
        showCrease: false, showUDC: false,
      };
    case 'ipad':
      return {
        topBarHeight: 26, bottomBarHeight: 0, borderRadius: 18,
        showTopBar: true, topBarStyle: 'ios-status',
        showDynamicIsland: false, showPunchHole: false, showNotch: false,
        showHomeIndicator: false, homeIndicatorStyle: 'ios',
        showCrease: false, showUDC: false,
      };
    case 'android-tablet':
      return {
        topBarHeight: 26, bottomBarHeight: 0, borderRadius: 14,
        showTopBar: true, topBarStyle: 'android-status',
        showDynamicIsland: false, showPunchHole: false, showNotch: false,
        showHomeIndicator: false, homeIndicatorStyle: 'android-pill',
        showCrease: false, showUDC: false,
      };
    case 'mac':
      return {
        topBarHeight: 28, bottomBarHeight: 0, borderRadius: 10,
        showTopBar: true, topBarStyle: 'mac-title',
        showDynamicIsland: false, showPunchHole: false, showNotch: true,
        showHomeIndicator: false, homeIndicatorStyle: 'ios',
        showCrease: false, showUDC: false,
      };
    case 'windows':
      return {
        topBarHeight: 32, bottomBarHeight: 0, borderRadius: 4,
        showTopBar: true, topBarStyle: 'windows-title',
        showDynamicIsland: false, showPunchHole: false, showNotch: false,
        showHomeIndicator: false, homeIndicatorStyle: 'ios',
        showCrease: false, showUDC: false,
      };
    case 'foldable':
      return {
        topBarHeight: 28, bottomBarHeight: 0, borderRadius: 14,
        showTopBar: true, topBarStyle: 'android-status',
        showDynamicIsland: false, showPunchHole: false, showNotch: false,
        showHomeIndicator: false, homeIndicatorStyle: 'android-pill',
        showCrease: true, showUDC: true,
      };
  }
}
...
const CanvasDecoration: React.FC<CanvasDecorationProps> = ({
  deviceType, canvasNode, canvasSectionEl,
}) => {
  ...
  useEffect(() => {
    if (!canvasNode) { setCanvasRect(null); return; }
    const m = () => { const r = canvasNode.getBoundingClientRect(); setCanvasRect({ left: r.left, top: r.top, width: r.width, height: r.height }); };
    m();
    const ro = new ResizeObserver(m); ro.observe(canvasNode); roCRef.current = ro;
    return () => { ro.disconnect(); roCRef.current = null; };
  }, [canvasNode]);

  useEffect(() => {
    if (!canvasSectionEl) { setSectionRect(null); return; }
    const m = () => { const r = canvasSectionEl.getBoundingClientRect(); setSectionRect({ left: r.left, top: r.top, width: r.width, height: r.height }); };
    m();
    const ro = new ResizeObserver(m); ro.observe(canvasSectionEl); roSRef.current = ro;
    return () => { ro.disconnect(); roSRef.current = null; };
  }, [canvasSectionEl]);
  ...
  const outerH = canvasRect.height + (config.showTopBar ? config.topBarHeight : 0) + (config.showHomeIndicator ? config.bottomBarHeight : 0);

  const battStyle = { background: `linear-gradient(to right, ${battery.color} ${battery.level}%, transparent ${battery.level}%)` };

  return (
    <div
      className={`${styles.decoration} ${styles[deviceType] ?? ''}`}
      style={{
        position: 'absolute', left: relLeft, top: relTop, width: outerW, height: outerH,
