...
const ConversationPanel: React.FC<ConversationPanelProps> = ({
  ...
  const isEmpty = !historyLoading && history.length === 0 && messages.length === 0 && !currentText && !loading
  ...
  return (
    <div ref={panelRef} className={styles.panel}>
      ...
      {isEmpty && (
        <div className={styles.welcomeWrap}>
          <div className={`${styles.bubble} ${styles.bubbleAssistant} ${styles.welcomeBubble}`}>
            <span className={styles.bubbleText}>👋 你好！我是你的 AI 设计助手。</span>
            <span className={styles.bubbleText}>告诉我你想要什么样的页面或效果，我来帮你实现。</span>
          </div>
        </div>
      )}
      ...
      {/* ── 初始思考中（无任何内容时） ── */}
      {loading && messages.length === 0 && !currentText && agentSteps.length === 0 && (
        <div className={styles.statusRow}>
      ...
      {hasPendingTask && !loading && (
        <div className={styles.pendingActions}>
          ...
          <div className={styles.pendingBtnGroup}>
            <button className={`${styles.pendingBtn} ${styles.pendingBtnConfirm}`} onClick={onConfirmTask}>
              <CheckOutlined />
              <span>确认保存</span>
            </button>
            <button className={`${styles.pendingBtn} ${styles.pendingBtnDiscard}`} onClick={onDiscardTask}>
              <CloseOutlined />
              <span>撤销修改</span>
            </button>
          </div>
...
const HistoryBubble: React.FC<{ message: ConversationMessage }> = ({ message }) => {
  ...
  return (
    <div className={`${styles.bubbleRow} ${isUser ? styles.bubbleRowUser : styles.bubbleRowAssistant}`}>
      <div className={`${styles.bubble} ${isUser ? styles.bubbleUser : styles.bubbleAssistant}`}>
        ...
        {!isUser && overflows && (
          <button className={styles.bubbleToggleBtn} onClick={() => setExpanded((v) => !v)}>
            {expanded ? '收起 ▲' : '展开 ▼'}
          </button>
        )}
      </div>
...
const ErrorRow: React.FC<{ message: ProgressMessage; onRetry?: (messageId: string) => void }> = ({
  message,
  onRetry,
}) => {
  ...
  return (
    <div className={styles.errorCard}>
      ...
      {canRetry && onRetry && (
        <button className={styles.errorCardRetryBtn} onClick={() => onRetry(message.id)} title="重试">
          <ReloadOutlined />
        </button>
      )}
