...
**Phase 2 — 逻辑层**（已完成）：云函数管理 + FlowSchema 后端执行。用户在 FunctionsPage 中通过可视化流程编辑器创建云函数（FlowSchema），可被页面组件事件绑定调用。后端通过 `@banyuan/flow/server` 的 `createServerFlowRunner()` 执行 FlowSchema。
...
│   ├── FlowRunnerService.ts     # FlowSchema 后端执行服务
...
│   ├── flows.ts          # FlowSchema 后端执行路由
│   ├── cloudFunctions.ts # 云函数 CRUD 路由
│   ├── build.ts          # 构建任务路由
│   ├── preview.ts        # 预览路由
│   ├── deploy.ts         # Web 部署路由
...
| GET/POST/PUT/DELETE | `/api/apps/:appId/cloud-functions[/:functionId]` | 云函数 CRUD |
