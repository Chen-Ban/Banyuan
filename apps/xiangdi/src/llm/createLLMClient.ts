import { DeepSeekClient, KimiClient, LLMRouter } from 'xiangdi'
import type { LLMClient, RoutingSignal } from 'xiangdi'
...
export async function getLLMRouter(): Promise<LLMRouter> {
    ...
    const deepseek = new DeepSeekClient({ apiKey: deepseekApiKey, model: deepseekModel, baseUrl: deepseekBaseUrl })
    ...
    const kimi = new KimiClient({ apiKey: kimiApiKey, model: kimiModel, baseUrl: kimiBaseUrl })
    ...
    _router = new LLMRouter({
        ...
        fallbacks: [
            { id: 'kimi', client: kimi, priority: 1 },
        ],
