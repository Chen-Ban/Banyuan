# BanvasDesign — 编辑态 React 绑定（`@banyuan/canvas-design`）
...
```ts
import { WorkerRuntime } from '@banyuan/canvas-design/worker'
```
...
| 入口 | 导入路径 | 内容 |
|------|----------|------|
| 主入口 | `@banyuan/canvas-design` | Hook + Actions + Workers + 交互系统 |
| Worker | `@banyuan/canvas-design/worker` | Worker 线程运行时（在 Web Worker 中导入） |
...
{
  "dependencies": {
    "@banyuan/canvas-design": "workspace:*"
  },
  "peerDependencies": {
    "react": ">=18",
...
本包依赖 `@banyuan/canvas`（核心引擎）和 `@banyuan/canvas-runtime`（共享的画布初始化逻辑）作为 peerDependencies，由宿主应用安装。
...
```bash
pnpm --filter @banyuan/canvas-design build
pnpm --filter @banyuan/canvas-design dev   # watch 模式
```
