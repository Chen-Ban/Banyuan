...
| 入口 | 导入路径 | 适用场景 |
|------|----------|----------|
| 编辑态 | `import ... from 'banvasgl'` | 低代码设计器（含 Worker、编辑 Hook） |
...
`FlowRunner` 将 `FlowSchema`（有向图 JSON）解释执行，支持顺序节点、条件分支（condition）、延迟（delay）、值节点（variable/pageVar/eventParam）。这是 Banyan 编辑器中"事件 → 交互逻辑"的运行时核心。
...
function DesignCanvas({ pages }) {
  const { Canvas, actions, selectedViewId } = useDesignBanvas(pages, {
    width: 800,
    height: 600,
  })
...
function RuntimeCanvas({ pages }) {
  const { Canvas } = useRuntimeBanvas(pages, { width: 800, height: 600 })
  return <div>{Canvas}</div>
}
```
