/**
 * IWorker —— Worker 通信协议公共类型
 *
 * WorkerTask / WorkerResult 被 core 层（TransactionManager 等）跨层引用，
 * 统一收录于此，避免 core/ 直接依赖 workers/ 实现层。
 *
 * WorkerHandler / WorkerHandlerResult 仅在 workers/ 内部使用，保留在原处。
 */
