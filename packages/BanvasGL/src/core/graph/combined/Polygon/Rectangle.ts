...
export default class Rectangle extends Polygon {
    ...
    constructor(x: number, y: number, width: number, height: number, style?: Style, options?: GraphOptions) {
        const vertices = [
            new Point3(x, y, 0),
    ...
    public intersects(other: Rectangle): boolean {
        ...
        return !(thisTopLeft.x + this.width < otherTopLeft.x ||
     ...
     * 从中心点创建矩形
     */
    public static createFromCenter(centerX: number, centerY: number, width: number, height: number, style?: Style): Rectangle {
