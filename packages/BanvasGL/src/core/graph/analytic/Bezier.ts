...
export default abstract class Bezier extends AnalyticGraph {
    ...
    constructor(controlPoints: Point3[], style: Style = Style.DEFAULT, options?: GraphOptions) {
        super(options)
    ...
    // 计算贝塞尔曲线的边界框（考虑曲线本身）
    protected calculateCurveBounds(steps: number = 50): { width: number; height: number; minX: number; maxX: number; minY: number; maxY: number } {
        let minX = Infinity, maxX = -Infinity
        let minY = Infinity, maxY = -Infinity
    ...
    // 获取贝塞尔曲线上最近的点
    getClosestPointOnCurve(point: Point3, steps: number = 100): { point: Point3; t: number; distance: number } {
    ...
    public getBezierType(): string {
        return this.controlPoints.length === 3 ? 'quadratic' : 
               this.controlPoints.length === 4 ? 'cubic' : 'unknown'
    }
}
