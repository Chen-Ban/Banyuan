 ...
 * Input 事件绑定
 */
export function useInputEvents({ inputRef, selectedView }: UseInputEventsOptions) {
  const isComposingRef = useRef<boolean>(false);

  // 处理普通文本输入（非合成输入）
  const onInput = useCallback(
    (e: Event) => {
      if (!isTextView(selectedView) || !selectedView.fixedIndex || !selectedView.dynamicIndex || !inputRef.current)
        return;
      ...
      if (e.inputType === "insertText") {
        ...
        if (insertedText.length > 0) {
          ...
          const newCursorPos = indexToCursorPos(selectedView, newIndex);
          inputRef.current.setSelectionRange(newCursorPos, newCursorPos);
        }
      }
      // 其他 inputType（删除、换行等）由 onKeyDown 处理
    },
    [selectedView, inputRef]
  );
  ...
  const onKeyDown = useCallback(
    (e: KeyboardEvent) => {
      if (!isTextView(selectedView) || !selectedView.fixedIndex || !inputRef.current) return;
