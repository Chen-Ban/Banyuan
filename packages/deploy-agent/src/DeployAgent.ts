...
import type { AgentConfig, AgentMessage, DeployRequest, DeployProgress, DeployResult } from './types.js'
...
export class DeployAgent {
  ...
  private async deployFullstack(request: DeployRequest): Promise<string> {
    ...
    await this.exec('docker', [
      ...
      '-e',
      `MONGODB_URI=${process.env.MONGODB_URI || 'mongodb://host.docker.internal:27017/banyuan_' + request.appSlug}`,
      '-e',
      `PORT=${containerPort}`,
      '--add-host',
