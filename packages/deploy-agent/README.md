...
1. **连接与认证**：agent 启动后连接 `BACKEND_WS_URL`，发送 `auth`（携带 `agentToken` + `tenantId`），并维持 30s 心跳；断线自动指数退避重连（5s → 60s）。
...
3. **脚手架化**：
   ...
   - `scaffoldServer()` 在全栈模式下生成后端服务器代码，内含 `flow-runner.js`——通过 `createServerFlowRunner()`（来自 `@banyuan/banvasgl/flow/server`）执行云函数的 FlowSchema。
...
| `scaffoldServer` | 脚手架化后端服务器（`ScaffoldServerOptions`） |
| 类型 | `AgentConfig`、`AgentMessage`、`DeployType`、`DeployRequest`、`DeployProgress`、`DeployResult`、`CollectionDef`、`CloudFunctionDef`、`AppJSON` 等 |
