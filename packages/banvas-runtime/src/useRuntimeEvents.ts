...
function hitTest(scene: Scene, point: Point3, bufferCtx: CanvasRenderingContext2D): View | null {
    let hit: View | null = null
    for (const view of scene.children) {
        const result = view.interact(point, bufferCtx)
...
export function useRuntimeEvents({ app, canvasRef }: UseRuntimeEventsOptions) {
    ...
    const onMouseDown = useCallback((e: MouseEvent) => {
        ...
        const view = hitTest(scene, point, bufferCtx)
        if (!view) return
    ...
    const onMouseMove = useCallback((e: MouseEvent) => {
        ...
        const bufferCtx = app.renderer.getCanvasContext().getBufferContext()
        const currentHover = hitTest(scene, point, bufferCtx)
        ...
        if (downPoint) {
            ...
            if (!isDraggingRef.current && dist >= DRAG_THRESHOLD) {
                ...
                const view = hitTest(scene, downPoint, bufferCtx)
