...
低级别 Hook，负责创建 Canvas 元素、App 实例、DPR 适配。通常不直接使用，由 `useRuntimeBanvas` 内部调用。也被 `@banyuan/canvas-design` 复用。
...
| 维度 | 运行态（本包） | 编辑态（@banyuan/canvas-design） |
|------|---------------|--------------------------------|
...
```bash
pnpm --filter @banyuan/canvas-runtime build
pnpm --filter @banyuan/canvas-runtime dev   # watch 模式
```
