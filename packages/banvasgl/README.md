...
| 包名 | npm | 职责 |
|------|-----|------|
| `packages/BanvasGL` | `@banyuan/canvas` | 核心引擎（本包） |
| `packages/BanvasDesign` | `@banyuan/canvas-design` | 编辑态 Hook + Workers + 交互分发 |
| `packages/BanvasRuntime` | `@banyuan/canvas-runtime` | 运行态 Hook（最小渲染集） |
...
| `packages/BanyanSDK` | `@banyuan/sdk` | 聚合 SDK（re-export 以上全部） |
...
```json
{ "dependencies": { "@banyuan/canvas": "workspace:*" } }
```

### 核心引擎（仅图形能力）

```ts
import { Scene, Serializer, ViewRegistry } from '@banyuan/canvas'
...
### 编辑态（需额外安装 @banyuan/canvas-design）

```tsx
import { useDesignBanvas } from '@banyuan/canvas-design'
...
### 运行态（需额外安装 @banyuan/canvas-runtime）
...
import { useFlowBanvas } from '@banyuan/sdk/flow'
import { Scene, Serializer } from '@banyuan/sdk/core'
```
...
```bash
pnpm --filter @banyuan/canvas build   # 生产构建
pnpm --filter @banyuan/canvas dev     # 开发模式（watch）
```
