...
export const mathExecutor: NodeExecutor = async (_node, inputs) => {
  ...
  switch (op) {
    case MathOp.Add:
      return { outputs: { value: a + b }, nextNodeId: null }
    case MathOp.Sub:
      return { outputs: { value: a - b }, nextNodeId: null }
    case MathOp.Mul:
      return { outputs: { value: a * b }, nextNodeId: null }
    ...
    case MathOp.Pow:
      return { outputs: { value: a ** b }, nextNodeId: null }
    case MathOp.Min:
      return { outputs: { value: Math.min(a, b) }, nextNodeId: null }
    case MathOp.Max:
      return { outputs: { value: Math.max(a, b) }, nextNodeId: null }
    default:
      return { outputs: { value: 0 }, nextNodeId: null }
  }
}
...
export const compareExecutor: NodeExecutor = async (_node, inputs) => {
  ...
  switch (op) {
    ...
    case CompareOp.Contains:
      return { outputs: { value: String(a).includes(String(b)) }, nextNodeId: null }
    default:
      return { outputs: { value: false }, nextNodeId: null }
  }
}
...
export const logicExecutor: NodeExecutor = async (_node, inputs) => {
  ...
  switch (op) {
    ...
    case LogicOp.Or:
      return { outputs: { value: !!(a || b) }, nextNodeId: null }
    case LogicOp.Not:
      return { outputs: { value: !a }, nextNodeId: null }
    default:
      return { outputs: { value: false }, nextNodeId: null }
  }
}
