...
export default class AnimationExecutor {
  ...
  constructor(descriptor: AnimationDescriptor, initialValues: KeyframeProps, hints?: InterpolationHints) {
  ...
  tick(timestamp: number): boolean {
    ...
    const overallProgress =
      desc.iterations === Infinity ? iterationProgress : Math.min(elapsed / totalDuration, 1)
  ...
  private _getFinalDirectedProgress(): number {
    const finalIteration =
      this.descriptor.iterations === Infinity ? 0 : Math.ceil(this.descriptor.iterations) - 1
