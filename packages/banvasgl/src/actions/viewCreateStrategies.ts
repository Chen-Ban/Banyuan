 ...
 * 默认 View 创建策略表
 *
 * 定义了根据 IComponentTemplate 创建具体 View 实例的工厂函数。
 * 作为 createBanvasActions 的内置默认策略。
 */
...
import { ImageElement, VideoElement } from '@/graph/media'
import { TextParagraph, TextFields } from '@/graph/text'
import Style from '@/foundation/style/Style'
import type Graph from '@/graph/base/Graph'
...
export const defaultViewCreatorStrategies = new Map<string, ViewCreatorStrategy>([
    [
        ViewType.GRAPHVIEW,
        (props, x, y) => {
            ...
            if (!graphStrategy) {
                throw new Error(`[BanvasGL] 未知 graphType "${graphType}"`)
            }
