...
export function createBanvasActions(
  ...
  return {
    ...
    template: {
      serialize: (viewId, config) =>
        _serializeTemplate(getApp, viewId, config),
      instantiate: (template, position, params?) =>
        _instantiateTemplate(getApp, template, position, params),
    },
  };
}
