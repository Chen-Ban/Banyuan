 ...
 */
export function collectIds(obj: any, idMap: Map<string, string>, nextPlaceholder: () => string): void {
...
function isFlowSchemaPath(path: string): boolean {
  return (
    ...
    path.startsWith('lifetimes.') ||
    path.includes('.events.') ||
    path.includes('.lifetimes.')
  )
}
 ...
 */
export function extractAssets(obj: any, assets: ITemplateAsset[], assetMap: Map<string, string>): void {
  if (obj === null || obj === undefined || typeof obj !== 'object') return
