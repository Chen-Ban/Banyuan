...
import type Vector3 from '@/foundation/math/Vector3'
import type View from '@/view/View/View'
import type { IGraph } from './graph/graph'
import type { IViewAddon } from './view/addon'
import type { ExtraData, Cursor } from './view/view'
import type SelectBoxView from '@/view/GraphViews/SelectBoxView'
import type EdgeView from '@/view/FlowViews/EdgeView'
...
/** 从联合中提取特定模式的状态类型 */
export type StateOfMode<M extends InteractionState['mode']> =
    Extract<InteractionState, { mode: M }>
...
export interface InteractionDelegate {
    ...
    // ── 文本选择 ──
    textInteract(view: View, point: Point3, bufferCtx: CanvasRenderingContext2D): { content: IGraph | IViewAddon | null }
    element2Index(view: View, content: IGraph | IViewAddon, point: Point3): any
    ...
    panMove(clientX: number, clientY: number, canvasWidth: number, canvasHeight: number): boolean
    panEnd(): boolean
    isSpaceHeld(): boolean
