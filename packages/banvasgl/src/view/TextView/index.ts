...
import type { ITextView } from '@/types/view/view'
import type { ISerializable } from '@/types/foundation/serializable'
import type { IInteractResult, ITextViewOptions } from '@/types/view/view'
...
export default class TextView extends View implements ITextView, ISerializable {
  ...
  public constraintPoint(relativePoint: Point3): Point3 {
    return Rectangle.fromBounds(this.content.bounds).getClosestPoint(relativePoint).closestPoint
  }
  ...
  protected interactContent(relativePoint: Point3, bufferCtx?: IDrawingContext): IInteractResult {
    // 是否命中文本域
    const hitedFields =
      this.content.isPointInPath(relativePoint, bufferCtx) || this.content.isPointOnCurve(relativePoint, 5)
  ...
  private deleteSelection(): TextIndex | undefined {
    ...
    if (startParagraphIndex === endParagraphIndex) {
      ...
      this.content.paragraphs.splice(startParagraphIndex + 1, paragraphsToDelete)
    ...
    if (isBackspace) {
      ...
      if (cursorIndex > 0) {
        ...
        if (fixedParagraphIndex > 0) {
          const prevParagraph = this.content.paragraphs[fixedParagraphIndex - 1]
      ...
      if (cursorIndex < paragraph.length) {
        ...
        if (fixedParagraphIndex < this.content.paragraphs.length - 1) {
          const currentParagraph = paragraph
          const nextParagraph = this.content.paragraphs[fixedParagraphIndex + 1]
  ...
  public newLine(): void {
    ...
    // 创建新段落，包含分割点后的内容
    const newParagraph = new TextParagraph([new NonPrintableTextElement()], paragraphOptions)
