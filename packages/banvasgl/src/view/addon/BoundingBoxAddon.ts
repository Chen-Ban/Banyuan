...
import type { IBoundingBoxAddon } from '@/types/view/addon'
...
export default class BoundingBoxAddon implements IBoundingBoxAddon {
  ...
  public capabilities = [AddonCapability.RENDER, AddonCapability.INTERACT, AddonCapability.LOGIC]
  ...
  interact(p: Point3, bufferCtx?: IDrawingContext): ExtraData | null {
    ...
    const isRotate = this.rotate[1].isPointOnCurve(p, 2) || this.rotate[1].isPointInPath(p, bufferCtx)
    const handler = this.handles.find((rec) => rec.isPointInPath(p, bufferCtx) || rec.isPointOnCurve(p, 5))
