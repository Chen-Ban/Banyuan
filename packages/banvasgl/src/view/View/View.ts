  ...
  implements IView, ISerializable
{
   ...
   */
  protected interactPlugins(relativePoint: Point3, bufferCtx?: IDrawingContext): IInteractResult {
   ...
   */
  public interact(worldPoint: Point3, bufferCtx?: IDrawingContext): IInteractResult {
  ...
  protected interactContent(point: Point3, bufferCtx?: IDrawingContext): IInteractResult {
    ...
    const hitContent = this.content.isPointInPath(point, bufferCtx) || this.content.isPointOnCurve(point, 5)
    if (hitContent) {
      return {
        view: this,
   ...
   * @returns 子视图的交互结果
   */
  protected interactChildren(_scrolledPoint: Point3, _bufferCtx: IDrawingContext): IInteractResult {
    return { view: null, content: null, extraData: null }
  }
  ...
  public render(surface?: IDrawingSurface): void {
    ...
    if (!surface) throw new Error('render failed: surface is required')
   ...
   */
  protected renderChildren(_surface: IDrawingSurface, _offscreenCtx: IDrawingContext): void {
    // 叶子 View 无子节点
  }
  ...
  public getWorldMatrix(parent?: View): Matrix4 {
    const localMatrix = this.animation?.resolveAnimatedMatrix() ?? this.matrix
  ...
  public layoutContent(ctx?: IDrawingContext): Bounds {
    return this.content?.layout(this.constraintBounds, ctx)?.bounds ?? this.content?.bounds ?? Bounds.empty()
  }
   ...
   */
  public resize(fixedPoint: Point3, dynamicPoint: Point3, vector: Vector3, needResizeContent?: boolean) {
    ...
    const deltaX = calculateDimensionDelta(viewport.width, referenceVector.x, relativeVector.x)
    const deltaY = calculateDimensionDelta(viewport.height, referenceVector.y, relativeVector.y)

    // 通过匹配 dynamicPoint 定位正在拖拽的手柄
    const dynamicIndex = handles.findIndex((h) => h.getCenter().isSame(dynamicPoint))
    if (dynamicIndex !== -1) {
  ...
  public getSnapObjects(): [Point3[], Line[]] {
    ...
    const points = this.boundingBox.handles.map((handler) => mvpInverse.multiply(handler.getCenter()))
    const lines = this.boundingBox.region.transform(mvpInverse).graphs as Line[]
    return [points, lines]
  }
}
