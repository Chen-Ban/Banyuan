...
class FlexLayoutStrategy implements ILayoutStrategy {
  public layout(children: View[], context: ILayoutContext, config: Record<string, any>): void {
    ...
    // 可用空间
    const availableMain = direction === 'row' ? viewport.width - pl - pr : viewport.height - pt - pb
    const availableCross = direction === 'row' ? viewport.height - pt - pb : viewport.width - pl - pr
    ...
    for (const child of children) {
      ...
      child.matrix = Matrix4.translation(tx + viewport.x, ty + viewport.y, 0)
