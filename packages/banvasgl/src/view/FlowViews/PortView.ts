...
import type { IPortView, PortDirection, IInteractResult, IViewOptions } from '@/types/index.js'
...
export default class PortView extends View implements IPortView {
  ...
  public getWorldCenter(): Point3 {
    ...
    const localCenter = new Point3(vp.x + vp.width / 2, vp.y + vp.height / 2, 0)
  ...
  protected interactContent(point: Point3, bufferCtx?: IDrawingContext): IInteractResult {
    ...
    const hit = this.content.isPointInPath(point, bufferCtx) || this.content.isPointOnCurve(point, 4)
    if (hit) {
      return {
        view: this,
