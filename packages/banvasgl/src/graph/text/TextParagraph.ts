...
export default class TextParagraph extends Graph implements ITextParagraph, ISerializable {
  ...
  get length(): number {
    ...
    if (length !== this.texts.length - 1) {
      console.warn('[TextParagraph] Text length is not equal to the number of printable text elements')
    }
    return length
  }
  ...
  public copy(): this {
    ...
    const newParagraph = new TextParagraph(texts, this.options.copy())
  ...
  static fromJSON(data: any): TextParagraph {
    ...
    const paragraph = new TextParagraph(texts, ParagraphOptions.fromJSON(data.options))
