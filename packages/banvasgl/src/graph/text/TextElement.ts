...
export default abstract class TextElement extends Graph implements ITextElement {
   ...
   * ```
   */
  constructor(content: string, options: TextOptions = TextOptions.DEFAULT, _style?: Style) {
    super()
  ...
  public updateBounds(): Bounds {
    if (this.isLayouted && this.controlPoints.length > 0) {
      ...
      const points = [
        startPoint,
        startPoint.add(new Vector3(this.width + this.options.letterSpacing, 0, 0)),
        startPoint.add(new Vector3(this.width + this.options.letterSpacing, this.lineHeight, 0)),
  ...
  public resize(fixedPoint: Point3, dynamicPoint: Point3, resizeVector: Vector3): void {
    this.options.size = Math.max(
      0,
      this.options.size +
        resizeVector.length * Math.sign(dynamicPoint.subtract(fixedPoint).y * resizeVector.y),
    )
  }
...
export class PrintableTextElement extends TextElement implements IPrintableTextElement, ISerializable {
   ...
   */
  public copy(): this {
    const newElement = new PrintableTextElement(super.content, super.options.copy())
  ...
  static fromJSON(data: any): PrintableTextElement {
    ...
    const el = new PrintableTextElement(content, TextOptions.fromJSON(data?.options))
