...
export default class TextFields extends Graph implements ITextFields, ISerializable {
  ...
  get textContent(): string[] {
    return this.paragraphs.map((paragraph) => paragraph.texts.map((text) => text.content).join(''))
  }
  ...
  public layout(constraintBounds?: Bounds, measureCtx?: IDrawingContext): TextFields {
    ...
    if (!this.options.fixedWidth) {
      const widths = this.paragraphs.map(
        (paragraph, i) =>
          paragraph.texts.reduce((a, b) => a + b.width + b.options.letterSpacing, 0) +
  ...
  private layoutParagraphs(paragraphs: TextParagraph[], layoutArea: Rectangle): void {
    // 从layoutArea左上角开始布局
  ...
  private layoutTextElementsInParagraph(
    ...
    const lineHeight = Math.max(
      ...lines.map((line) => this.calculateLineHeight(line, paragraph.options.leading)),
    )
  ...
  private adjustParagraphHorizontalAlignment(paragraph: TextParagraph, maxWidth: number): void {
    ...
    for (const textElement of paragraph.texts) {
      ...
      textElement.applyLayout(textElement.controlPoints[0], textElement.lineHeight)
    }
  }
  ...
  private adjustParagraphVerticalAlignment(paragraphs: TextParagraph[], layoutArea: Rectangle): void {
    ...
    for (const paragraph of paragraphs) {
      for (const textElement of paragraph.texts) {
        textElement.applyLayout(textElement.controlPoints[0].add(offsetVector), textElement.lineHeight)
      }
    }
  }
  ...
  public point2TextElement(relativePoint: Point3, bufferCtx?: IDrawingContext | null): TextElement | null {
    const hitedParagraph = this.paragraphs.find(
      (paragraph: TextParagraph) =>
        paragraph.isPointInPath(relativePoint, bufferCtx) || paragraph.isPointOnCurve(relativePoint, 5),
    )
    if (hitedParagraph) {
      // 准确命中某个文字
    ...
    const hitedLayoutArea =
      layout.isPointInPath(relativePoint, bufferCtx) || layout.isPointOnCurve(relativePoint, 5)
    if (hitedLayoutArea) {
   ...
   */
  public element2Index(textElement: TextElement, relativePoint: Point3): TextIndex {
   ...
   */
  private probeTextElement(textElements: TextElement[], p: Point3): TextElement | null {
  ...
  public copy(): this {
    ...
    const newTextFields = new TextFields(copiedParagraphs, this.options.copy())
  ...
  static fromJSON(data: any): TextFields {
    ...
    const fields = new TextFields(paragraphs, TextFieldsOptions.fromJSON(data.options))
    fields.id = data.id
    return fields
  }
   ...
   * 静态工厂方法 - 创建多段落文本域
   */
  static fromStrings(contents: string[], options?: TextFieldsOptions): TextFields {
