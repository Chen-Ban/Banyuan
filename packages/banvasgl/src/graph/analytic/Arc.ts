...
export default class Arc extends AnalyticGraph implements IArc, ISerializable {
  ...
  public updateBounds(): Bounds {
    ...
    // 极值候选角度：两个 x 极值 + 两个 y 极值
    const candidates: number[] = [thetaX, thetaX + Math.PI, thetaY, thetaY + Math.PI]
    ...
    // 起止点必须包含
    const points: Point3[] = [pointAtTheta(this.startAngle), pointAtTheta(this.endAngle)]
  ...
  public getPointAt(t: number): Point3 {
    ...
    // 应用旋转和平移
    const rotated = Matrix4.identity().rotateZ(this.rotation).multiply(localPoint)
    return rotated.add(new Vector3(this.center.x, this.center.y, this.center.z))
  }
