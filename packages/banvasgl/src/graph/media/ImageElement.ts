...
export default class ImageElement extends MediaElement implements IImageElement, ISerializable {
   ...
   */
  public copy(): this {
    const copy = new ImageElement(this.src, this.x, this.y, this.width, this.height)
    return copy as this
  }
  ...
  static fromJSON(data: any): ImageElement {
    const el = new ImageElement(data.src, data.x, data.y, data.width, data.height)
    el.id = data.id
    return el
  }
  ...
  static fromCanvas(
    ...
    return Promise.resolve(ImageElement.fromImageSource(source, x, y, width, height, _style))
  }
}
