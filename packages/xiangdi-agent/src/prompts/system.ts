...
export function generateNodeSchemaDoc(): string {
  ...
  lines.push(
    `- { type: "gradient", direction: "horizontal"|"vertical"|"diagonal", stops: [{ offset: 0-1, color: Color }] } — 渐变填充`,
  )
  lines.push(`- { type: "none" } — 无填充`)
  lines.push(``)
  lines.push(
    `**描边（Stroke）**：可选，{ color: Color, width: number（≥0）, style: "solid"|"dashed"|"dotted"（默认 "solid"）}`,
  )
  lines.push(``)
  lines.push(
    `**文本样式（TextStyle）**：{ fontSize: number（默认 14）, fontWeight: "normal"|"bold"（默认 "normal"）, color: Color（默认 "#000000"）, align: "left"|"center"|"right"（默认 "left"）, lineHeight: number（默认 1.5）}`,
  )
  lines.push(``)
  lines.push(`### 节点类型`)
  lines.push(``)
  ...
  lines.push(``)
  lines.push(
    `**Page**：{ id: string, name: string（默认 "页面"）, width: number（默认 375）, height: number（默认 812）, backgroundColor: Color（默认 "#ffffff"）, nodes: Node[] }`,
  )
  lines.push(``)
  lines.push(
    `**App**：{ id: string, name: string, scenes: Scene[]（至少 1 个）, version: string（默认 "1.0.0"）}`,
  )
