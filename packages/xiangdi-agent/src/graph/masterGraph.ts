...
const AUDIT_SYSTEM_PROMPT = `你是一个 UI 页面审计专家。给定生成的页面内容和用户的原始需求，验证结果是否满足风格和意图要求。

注意：格式合法性、数值范围等硬性规则已由代码层校验通过，你只需关注以下软性维度：

验证维度（按优先级）：
1. 用户本轮风格需求：用户在本次对话中明确表达的风格/外观/体验要求是否满足
2. 用户偏好一致性：结果是否与用户已知的设计偏好一致（如有偏好信息会在上下文中提供）
3. 需求完整性：用户要求的功能和元素是否都已体现
4. 视觉协调性：整体风格是否协调统一（配色、间距、圆角、字体搭配）
5. 交互逻辑：事件绑定和流程是否合理
...
export function createMasterGraph(config: MasterGraphConfig) {
  ...
  function isTransientLLMError(err: unknown): boolean {
    ...
    const patterns = [
      'econnreset', 'econnrefused', 'etimedout', 'timeout',
      'socket hang up', 'network', 'aborted',
  ...
  async function retryableLLMCall<T>(fn: () => Promise<T>): Promise<T> {
    ...
    for (let attempt = 0; attempt <= LLM_MAX_RETRIES; attempt++) {
      try {
        ...
        const delay = LLM_INITIAL_DELAY_MS * Math.pow(2, attempt);
        console.warn(
          `[XiangDi] LLM transient error (attempt ${attempt + 1}/${LLM_MAX_RETRIES + 1}), ` +
  ...
  async function executeTask(
    ...
    while (iteration < maxIter) {
      ...
      // Think: Call LLM（流式，逐 token 推送 text_delta；含瞬时错误自动重试）
      const xiangdiMessages = langchainToXiangdi(currentMessages);
      let streamedText = "";
      const response = await retryableLLMCall(() => llmClient.createMessageStream(
        {
          model: "deepseek-v4-pro",
          max_tokens: 8192,
  ...
  async function auditNode(state: MasterState): Promise<Partial<MasterState>> {
    ...
    // ─── Phase 1: 硬性规则前置校验（零 token 成本）──────────────────────────
    const ruleIssues = ruleBasedAudit(state);
    if (ruleIssues.length > 0) {
      // 硬性规则失败 → 直接 fail，不调用 LLM
      const auditResult: AuditResult = { passed: false, issues: ruleIssues };
      const auditErrorSummary = buildAuditErrorSummary(auditResult);
      const shouldSummarize = state.auditRetries + 1 >= maxAuditRetries;
    ...
    // ─── Phase 2: 风格与意图校验（LLM，每次必做）───────────────────────────
    // 三层优先级：用户本轮需求 > 记忆中偏好 > 系统兜底
    const intentSummary = state.planOutput?.intentSummary ?? "";
    const auditPrompt = [
      `## 用户原始需求\n${intentSummary}`,
      `## 执行方案\n${state.planOutput?.planDescription ?? "N/A"}`,
      state.agentMemory ? `## 用户偏好（来自记忆）\n${extractPreferenceFromMemory(state.agentMemory)}` : null,
      `## 执行结果摘要\n${state.finalText.slice(0, 3000)}`,
      `\n硬性规则已全部通过。请从风格、意图匹配、视觉协调性等软性维度验证。`,
    ].filter(Boolean).join("\n\n");
...
/**
 * ADR-035 Phase 1: 硬性规则审计（零 token 成本）
 * 检查工具调用成功率、布局数值合法性、格式正确性等可编码维度。
 * 返回发现的 issues 列表（空 = 全通过）。
 */
function ruleBasedAudit(state: MasterState): AuditResult["issues"] {
  const issues: AuditResult["issues"] = [];

  // 检查 finalText 中的工具调用结果错误标记
  // finalText 中若包含 is_error 标记说明有工具执行失败
    ...
    try {
      ...
      if (result && typeof result === "object" && result.is_error === true) {
        issues.push({
          ...
          message: `工具调用失败: ${(msg as ToolMessage).name ?? "unknown"} - ${result.message ?? content.slice(0, 100)}`,
          suggestion: "检查工具参数是否正确",
        });
      }
    } catch {
      // 非 JSON 格式的 ToolMessage 内容，跳过
    }
  }

  // 从 finalText 中提取视图快照做布局校验（如果是结构化 JSON）
  if (state.finalText) {
    try {
      const result = JSON.parse(state.finalText);
      if (result && Array.isArray(result.views)) {
        for (const view of result.views as Array<Record<string, unknown>>) {
          const { x, y, width, height, id } = view as { x?: number; y?: number; width?: number; height?: number; id?: string };
          const viewId = (id as string) ?? "unknown";
          if (typeof width === "number" && width <= 0) {
            issues.push({ severity: "error", category: "layout", message: `视图 ${viewId} 的 width(${width}) 无效`, suggestion: "width 必须为正数" });
          }
          if (typeof height === "number" && height <= 0) {
            issues.push({ severity: "error", category: "layout", message: `视图 ${viewId} 的 height(${height}) 无效`, suggestion: "height 必须为正数" });
          }
          if (typeof x === "number" && x < 0) {
            issues.push({ severity: "warning", category: "layout", message: `视图 ${viewId} 的 x(${x}) 为负值`, suggestion: "检查是否超出画布边界" });
          }
          if (typeof y === "number" && y < 0) {
            issues.push({ severity: "warning", category: "layout", message: `视图 ${viewId} 的 y(${y}) 为负值`, suggestion: "检查是否超出画布边界" });
          }
          // 颜色格式校验
          const bgColor = view.backgroundColor;
          if (typeof bgColor === "string" && bgColor && !/^#[0-9a-fA-F]{3,8}$/.test(bgColor) && !/^rgba?\(/.test(bgColor)) {
            issues.push({ severity: "error", category: "style", message: `视图 ${viewId} 的 backgroundColor(${bgColor}) 格式无效`, suggestion: "使用 #RRGGBB 或 rgba() 格式" });
          }
        }
      }
    } catch {
      // finalText 不是 JSON 格式，跳过布局校验
    }
  }
...
/**
 * 构建审计错误摘要（从重复代码提取为独立函数）
 */
function buildAuditErrorSummary(auditResult: AuditResult): string {
...
/**
 * 从 agentMemory 文本中提取用户偏好段落（供 auditNode 风格校验使用）
 */
function extractPreferenceFromMemory(agentMemory: string): string {
  // agentMemory 是多段文本，尝试提取包含 user_preference 或偏好相关的内容
  const lines = agentMemory.split("\n");
  const preferenceLines: string[] = [];
  const keywords = ["偏好", "preference", "风格", "style", "喜欢", "prefer", "圆角", "配色", "字体", "间距", "布局", "主题", "theme"];
  for (const line of lines) {
    const lower = line.toLowerCase();
    if (keywords.some((k) => lower.includes(k))) {
      preferenceLines.push(line.trim());
    }
  }
...
/**
 * ADR-035: summarizeNode 增强 Phase 1
 * 从 state 各节点核心产出中提取结构化改动信息。
 */
function extractChangeSummaryInput(state: MasterState): string {
  const sections: string[] = [];

  // 1. 用户意图（来自 planOutput）
  if (state.planOutput?.intentSummary) {
    sections.push(`## 用户需求\n${state.planOutput.intentSummary}`);
  }

  // 2. 方案要点（来自 planOutput.planDescription，截断 300 字）
  if (state.planOutput?.planDescription) {
    const desc = state.planOutput.planDescription;
  ...
  if (state.auditResult) {
    ...
    const issueDesc = state.auditResult.issues
      .filter((i) => i.severity !== "info")
      .slice(0, 5) // 最多列 5 条
      .map((i) => `${i.severity}: ${i.message}`)
      .join("; ");
    sections.push(`## 审计结果\n${status}${issueDesc ? "。问题: " + issueDesc : ""}`);
  }
...
/**
 * 从 messages 中提取 ToolMessage，翻译为人类可读的语义动作列表
 */
function extractToolActions(messages: BaseMessage[]): ToolAction[] {
  ...
  for (const msg of messages) {
    ...
    const action = parseToolAction(toolName, parsed);
...
/**
 * 将 BanvasToolProtocol 工具名+结果映射为语义动作。
 * 映射表与 BanvasToolProtocol 工具同步维护。
 */
function parseToolAction(toolName: string, result: Record<string, unknown>): ToolAction | null {
  switch (toolName) {
    case "banvas_create_page":
      return { action: "新建页面", target: `"${result.name ?? result.id ?? "unknown"}"` };
    case "banvas_add_node":
      return {
        action: "创建",
        target: `${result.viewType ?? "View"} "${result.name ?? result.id ?? "unknown"}"`,
        detail: result.parentId ? `在 ${result.parentId} 下` : undefined,
      };
    case "banvas_update_node":
      return {
        action: "修改",
        target: `"${result.nodeId ?? result.id ?? "unknown"}"`,
        detail: result.properties ? summarizeProperties(result.properties as Record<string, unknown>) : undefined,
      };
    case "banvas_delete_node":
      return { action: "删除", target: `"${result.nodeId ?? result.id ?? "unknown"}"` };
    case "banvas_move_node":
      return { action: "移动", target: `"${result.nodeId ?? result.id ?? "unknown"}"`, detail: `到 (${result.x}, ${result.y})` };
    case "banvas_resize_node":
      return { action: "调整尺寸", target: `"${result.nodeId ?? result.id ?? "unknown"}"`, detail: `${result.width}×${result.height}` };
    case "banvas_apply_patch":
      return { action: "批量更新", target: `${Array.isArray(result.patches) ? (result.patches as unknown[]).length : "?"} 项变更` };
    case "banvas_get_app_state":
      return null; // 读取操作不记入改动
    case "generate_cloud_function":
      return { action: "创建云函数", target: `"${result.name ?? "unknown"}"` };
    case "update_cloud_function":
      return { action: "更新云函数", target: `"${result.name ?? "unknown"}"` };
    case "schema_set_collections":
      return { action: "设置数据模型", target: `${Array.isArray(result.collections) ? (result.collections as unknown[]).length : "?"} 个集合` };
    case "web_search":
    case "knowledge_search":
    case "material_search":
    case "material_get_detail":
    case "schema_get":
    case "explain_cloud_function":
      return null; // 查询/搜索操作不记入改动
    default:
      // 未知工具：用原始名+截断内容
      return { action: toolName, target: JSON.stringify(result).slice(0, 80) };
  }
}

/**
 * 将 properties 变更对象摘要为短文本
 */
