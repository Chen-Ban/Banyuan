...
const LIFETIME_KEYS: (keyof AILifetimes)[] = ['onCreated', 'onAttach', 'onDestroy']
...
export function uiJSONToProjection(uiJSON: string): AIProjectionApp {
  ...
  const scenes: AIProjectionScene[] =
    appData && Array.isArray(appData.scenes)
...
function projectGraphView(base: BaseFields, data: any): AIGraphViewNode {
  const content = data.content ? { graphType: data.content.$type, data: data.content.$value ?? {} } : null
  return { ...base, type: 'GRAPHVIEW', content }
}

function unprojectGraphView(node: AIGraphViewNode): Record<string, unknown> {
  return {
    content: node.content ? { $type: node.content.graphType, $value: node.content.data } : null,
    children: [],
  }
}
...
const BANVAS_ALIGN_TO_AI: Record<string, 'left' | 'center' | 'right'> = {
  LEFT: 'left',
  CENTER: 'center',
  RIGHT: 'right',
}
...
function unprojectGenericView(node: AIGenericViewNode): Record<string, unknown> {
  ...
  result.children = node.children ? node.children.map((child) => unprojectNode(child)) : []
  return result
}
...
function unprojectMatrix4(transform: AITransform): any {
  ...
  return {
    transform: [scaleX * cos, scaleY * sin, 0, x, scaleX * -sin, scaleY * cos, 0, y, 0, 0, 1, 0, 0, 0, 0, 1],
  }
}
