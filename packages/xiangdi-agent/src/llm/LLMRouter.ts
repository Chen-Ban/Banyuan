...
export class LLMRouter implements LLMClient {
  ...
  private recordSuccess(health: ProviderHealth, latencyMs: number): void {
    ...
    health.avgLatencyMs = health.latencyWindow.reduce((a, b) => a + b, 0) / health.latencyWindow.length
  }
...
function normalizeLLMProvider(input: LLMProvider | LLMClient, defaultId: string): LLMProvider {
...
function classifyError(error: Error, latencyMs: number, timeoutThreshold: number): RoutingSignalType {
...
function checkResponseQuality(response: LLMResponse): string | null {
  ...
  const hasNonEmptyText = response.content.some((c) => c.type === 'text' && c.text.trim().length > 0)
