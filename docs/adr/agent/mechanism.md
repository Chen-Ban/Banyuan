...
知识种子 JSON 文件入 Git 仓库（已实施），通过 HTTP API 幂等写入 knowledge-server（已实施）。CI 守护 Freshness 未实施。

**实现（已完成部分）：** `apps/knowledge-server/scripts/seed-knowledge.ts`

种子写入脚本能力：

- 从 `packages/xiangdi-agent/src/knowledge/seeds/` 读取 JSON 文件
- 通过 HTTP API（`POST /knowledge/upsert`）写入 knowledge-server
- 支持分层写入：`--layer schema|composition|theme|all`
- 幂等执行：先删旧再写新（非原子，但嵌入式 DB 够用）

**未实施部分：** CI knowledge-guard（Primitive 过期 blocking + Composition 影响分析 non-blocking）

**决策链：** 引擎代码演进时知识必须同步 → 依赖开发者记忆不可靠 → CI 自动检测是最后防线 → 种子入 Git 可以 Code Review、git blame、做影响分析。

**约束：**

- Primitive Seeds 过期应 block PR（CI knowledge-guard job 检测 content hash 不匹配）
