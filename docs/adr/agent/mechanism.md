...
- 三维度分离记忆（Preferences + Anchor + Raw Messages）——Anchor 维度被废除：锚点试图静态存储"当前应用关键状态"，但应用状态随时变化，静态锚点会过时。改为从当前 uiJSON 动态生成上下文摘要
...
**概念定义：** Checkpoint 是流程进度的状态快照（OrchestratorState，核心是 artifacts 五槽：requirements/uiDesign/contract/frontend/backend），按 thread_id 持久化。它记录的是「流程走到哪了、各阶段产出了什么」，**不是上下文**——应用状态（uiJSON/schema/cloudFunctions）每次请求动态拉取，对话历史由 banyan 后端通过 previousMessages 传入，二者均不依赖 Checkpoint。
...
**设计动机：** 原始 `projectionToUIJSON(scenes, version)` 在还原 uiJSON 时硬编码 `lifetimes: { onLaunch: null, onUnlaunch: null }`。如果应用已设置了 App 级生命周期（如"启动时初始化数据"），经过 AI 操作一轮 roundtrip 后 App 级 lifetimes 会被清空——数据丢失。根因是 `AIProjectionScene[]` 只建模了 Scene（页面）级别数据，没有建模 App 级元数据。当 `projectionToUIJSON` 从 scenes 数组重建 uiJSON 时，缺少 App 级 lifetimes 的来源。patchProjection 的引入将写入语义从全量覆盖改为增量 patch——只更新涉及的 scene，保留其他所有内容（包括 App-level lifetimes）。
...
**决策链：** 前端 Worker 通过 write_page 工具写入单页 AIProjection，早期 `projectionToUIJSON` 是全量覆盖语义（传入的 scenes 替换 uiJSON 中所有 scenes）。问题：Worker 每次只写一页，全量覆盖会丢失其他页面和 App-level 配置（lifetimes）。patchProjection 将语义改为增量 patch——只更新涉及的 scene，保留其他所有内容。
...
- projectionToUIJSON 全量覆盖——多页面应用中写入一页会丢失其他页面
