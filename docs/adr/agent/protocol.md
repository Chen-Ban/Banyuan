...
**去掉 baseUIJSON 概念：** Dialogue 不再存储 baseUIJSON（执行前基线快照）。理由：每个 done 状态的 Dialogue 的 uiJSON 字段本身就是该轮对话完成后的应用状态快照。Dialogue 链天然形成版本历史。用户拒绝验收时当前 Dialogue 产出被丢弃、Application 表不变（等同于什么都没发生）。
...
- Dialogue 链天然形成版本历史（每个 done 的 Dialogue 的 uiJSON 是该轮完成后的快照）
...
- `app_state` 保留为独立事件（在 xiangdi-server 层注入，不在 Orchestrator 图内），用于请求结束时向 banyan 后端推送最终 uiJSON/schema/cloudFunctions
...
- 统一命名风格：动词*名词，去掉 banvas* 前缀（不再需要区分画布工具和其他工具）
...
2. xiangdi-server 通过 BanyanClient 并行拉取：`GET /internal/apps/:appId/json`（uiJSON）、`GET /internal/apps/:appId/schema`（CollectionSchema）、`GET /internal/apps/:appId/functions`（CloudFunctions）
3. 数据加载到 AppRuntimeState（内存对象），Worker 工具在其上读写
4. Orchestrator 图执行完毕后，xiangdi-server 推送 `app_state` SSE 事件（含最终 uiJSON + schema + cloudFunctions）
...
- xiangdi-server 不持有任何业务数据（uiJSON 随请求进出，不落盘）
