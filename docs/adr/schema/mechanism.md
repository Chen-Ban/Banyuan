...
- M1、M2、M3 分别服务于不同的数据生命周期阶段：M1 处理 AI↔引擎的格式桥接，M2 处理跨版本的格式升级，M3 处理用户级别的片段复用。三者互相独立，各自细化架构级决策 A1（全量 JSON 基座）的不同投影维度。
