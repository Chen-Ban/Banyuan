...
- engine:A0→P1：A0（engine 域）确立「runtime 只提供机制不内置策略」，P1「引擎不内置兼容旧格式分支、迁移外置」是该契约在版本兼容维度的直接落地，故 P1 refines engine:A0。
...
**未实施** · refines engine:A0 · 驱动 M2（数据迁移外置）

BanvasGL 引擎代码中不包含任何"如果是旧格式则..."的兼容分支。引擎假设输入数据永远是当前版本格式，格式升级的责任外置到 Migration 层。

> **A0 机制/策略定位：** 本原则是 A0 机制/策略分离契约在版本兼容维度的直接落地。banvasgl 运行时只提供「当前版本格式的序列化/反序列化机制」与「版本迁移机制」（MigrationRegistry），把「如何从旧版升级到当前版」的转换规则与「何时执行迁移」的编排都视为外置策略——运行时内核不内置「兼容旧格式」分支，正是「runtime 只提供机制不内置策略」的体现，故 refines engine:A0。

**决策链：** A0 确立运行时只提供机制不内置策略 → 把「兼容旧格式」做成引擎内分支即是在内核里内置版本策略 → 兼容代码随版本累积会形成不可维护的债务 -> 每个兼容分支都需要测试、都可能引入 bug -> 职责分离：引擎专注当前版本格式机制的正确性，Migration 策略专注旧->新的转换正确性。
