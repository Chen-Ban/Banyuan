...
│                          动画系统                                            │
│  ┌────────────────┐     ┌────────────────┐     ┌────────────────┐           │
│  │M11 Descriptor  │────▶│M12 Executor    │────▶│M13 Manager     │           │
│  │  动画描述对象   │     │  关键帧插值     │     │  全局帧驱动     │           │
│  └────────────────┘     └────────────────┘     └────────────────┘           │
└─────────────────────────────────────────────────────────────────────────────┘
...
│                          流程执行                                            │
│  ┌────────────────┐     ┌────────────────┐     ┌────────────────┐           │
│  │M14 FlowSchema  │────▶│M15 Scene 生命  │     │M16 View 事件   │           │
...
- M7→M8：布局脏标记驱动两阶段管线
- M9⇄M10：坐标系统与吸附对齐互补（命中检测提供坐标转换，吸附对齐提供位置修正）
- M11→M12→M13：动画系统三层递进（描述→计算→调度）
- M14→M15/M16：FlowSchema 执行机制是生命周期和事件绑定的共同基础
- M17 独立：外部订阅是引擎与 React 宿主的桥接机制
...
### M11. AnimationDescriptor 动画描述对象
...
### M12. AnimationExecutor 关键帧插值计算器
...
### M16. View 事件绑定 FlowSchema（13 个事件处理器）
...
### M17. useSyncExternalStore 外部订阅机制
