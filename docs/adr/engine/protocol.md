...
```
┌──────────────────────┐          ┌──────────────────────────┐
│ C2 Renderer 接口协议  │          │ C1 FlowContext 构造与传递 │
└──────────────────────┘          └────────────┬─────────────┘
                                               │ enables
                                  ┌────────────▼─────────────┐
...
┌──────────────────────────────────┐
│ C3 App ↔ Scene ↔ View 通信协议   │
└───────┬──────────┬───────────┬───┘
        │          │           │
        │ enables  │ enables   │ enables
        │          │           │
┌───────▼───┐ ┌───▼────────┐ ┌▼──────────────────┐
│C4 事务提交 │ │C5 Hook 层  │ │C7 View 序列化协议  │
│   协议     │ │  通信协议   │ │                   │
└───────────┘ └────────────┘ └───────────────────┘
```
...
- C1→C6：FlowContext 是 NodeExecutor 的执行环境，注册协议依赖 Context 定义
- C3→C4：事务提交协议操作的对象是 App/Scene/View 树
- C3→C5：Hook 层桥接的是 App 实例与 React 宿主
- C3→C7：序列化协议将 View 树持久化为 JSON
- C2 独立：Renderer 接口是渲染层的独立抽象

---

## 流程执行协议

### C1. FlowContext 构造与传递协议
