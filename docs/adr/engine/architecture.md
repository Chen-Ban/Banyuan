...
- A8b→A9：平台抽象接口作为天然 FFI 边界，使 Rust 2D 全量改写成为可能（A9 refines A8b）。改写分为两步：第一步完成 TypeScript → Rust 的全量 2D 移植并验证功能对等，第二步在已验证的 Rust 2D 核心之上将渲染管线重构为 wgpu 驱动的 GPU 渲染
- A9→A9a：Step1 2D Rust 核心验证完成后，Step2 将渲染管线从 CPU 软件绘制重构为 wgpu 驱动的 GPU 渲染（A9a refines A9）
...
actions（封装操作 API）
  ↑
engine（App/Scene/Renderer/Camera/TransactionManager/material/serialization）
  ↑
view（View 基类 + 子类 + addon + property）
  ↑
graph（图形基元：形状/文本/媒体/轨迹）
  ↑
foundation（数学/样式/动画/常量/工具 + flow 运行时，仅依赖 types）
  ↑
types（纯接口契约，零实现；flow 类型在 src/types/foundation/flow/）
```

flow 运行时在 `src/foundation/flow/` 中，仅依赖 types 层，与 foundation 内其他模块（math/style/animation）共处同一层。graph 层依赖 foundation + types，view 层依赖 graph + foundation，engine 层依赖 view + graph + foundation（含 flow），actions 封装对 engine/view 的复合操作。六层依赖方向严格自下而上。

**决策链：** A0 确立图形运行时定位（runtime ⊃ rendering + 机制/策略分离）→ 六层划分是该运行时的内部结构，承载机制实现（types / foundation / graph / view / engine / actions）→ 引擎需要服务多种场景（前端渲染/后端流程执行/React 集成/独立测试）→ 分层隔离各职责 → 单一入口 index.ts 统一导出公共 API → 子路径导出服务特定场景。React 宿主集成（useCanvasInit 等 hook + Web 平台适配器）抽取为独立包 @banyuan/banvasgl-react。

**约束：**

- types 是纯接口，不含实现代码；flow 类型在 `src/types/foundation/flow/`；类型守卫在 `src/foundation/guards.ts`
- foundation 是零依赖原子模块（数学/样式/动画/常量/工具 + flow 运行时），仅依赖 types；flow 实现在 `src/foundation/flow/`；Flow 类型在 `src/types/foundation/flow/`
- graph 层依赖 foundation + types
- view 层依赖 graph + foundation
- engine 层是运转核心，依赖 view + graph + foundation（含 flow）
- actions 封装对 engine/view 的复合操作
- React 宿主集成（useCanvasInit / useCanvasCamera / WebDrawingContext 等）在独立包 @banyuan/banvasgl-react 中，声明 React 为 peerDep；banvasgl 核心零 DOM / 零 React 依赖
...
- 子路径导出：包级 exports 仅有三个子路径：`.`（主入口，含 Flow 类型）、`./flow/client`、`./flow/server`；**不存在 `./flow` 公开子路径**——内部组件（FlowRunner 类、NodeExecutor 注册表、各求值器）不对外暴露，只暴露预组装工厂。React Hook 在独立包 @banyuan/banvasgl-react 中
...
引擎核心零 DOM / 零 React 依赖，通过 `subscribe` / `getVersion` / `notify` 三件套暴露外部状态订阅能力（`useSyncExternalStore` 模式）。React 宿主集成（`useCanvasInit` / `useCanvasCamera` / Web 平台适配器）全部抽取到独立包 `@banyuan/banvasgl-react` 中。

**决策链：** 引擎不依赖 React，但需要与 React 宿主协同 → 引擎暴露状态订阅机制（subscribe/getVersion/notify） → React hook 在独立包中消费该机制 → banvasgl 核心保持平台无关，React/Web 绑定随 banvasgl-react 包独立演进。

**约束：**

- banvasgl 核心不 import React，不含任何 hook 实现
- `@banyuan/banvasgl-react` 提供：`useCanvasInit`（App 生命周期 + DOM 结构 + ResizeObserver + DPR 响应 + appJSON 反序列化）、`useCanvasCamera`（相机驱动的无限画布交互）、`WebDrawingContext` / `WebSurface`（Web 平台适配器）
...
三态（编辑态/预览态/线上态）全部使用 @banyuan/banvasgl 同一个引擎包，通过上层注入不同的交互策略和 `flowEnabled` 配置控制行为边界。
...
- 三态共享 `@banyuan/banvasgl-react` 的 `useCanvasInit`（Canvas DOM 初始化、Renderer 创建、Camera 设置）
- 编辑态通过 banyan 前端的 `useDesignBanvas` 注入编辑策略（InteractionStateMachine）+ `flowEnabled=false`
- 预览态/线上态通过 `@banyuan/banvas-react-runtime` 的 `useRuntimeBanvas` 注入运行策略（高级交互识别 + View.events 派发）+ `flowEnabled=true`
...
### A8b. 平台抽象层：IDrawingContext / IDrawingSurface

**✅ 已实施** · refines A8（从 React-only 宿主集成泛化为平台无关抽象）

A8 的宿主集成已通过 banvasgl-react 包覆盖 React + Web 场景。跨平台（iOS/Android/Node 服务端渲染）要求引擎核心不持有任何平台特定类型（HTMLCanvasElement、CanvasRenderingContext2D、OffscreenCanvas）。

本决策将宿主集成从「React hook 桥接」升级为「平台无关接口注入」，将 React/Web 绑定集中在 @banyuan/banvasgl-react 中，banvasgl 核心彻底零 DOM 依赖。

**决策链：** A0 确立机制/策略分离 → A8 建立 React hook 宿主集成 → 跨平台需求要求引擎不依赖 DOM 类型 → 定义平台无关绘图接口 IDrawingContext（替代 CanvasRenderingContext2D）→ 定义平台画布注入接口 IDrawingSurface（替代 HTMLCanvasElement + OffscreenCanvas + 帧调度，合并原 IPlatformCanvas + IEngineContext）→ React/Web 绑定抽取为独立包 @banyuan/banvasgl-react。

**两个核心接口：**

| 接口              | 定义位置                    | 职责                                                                      | 对应 Web 实现                        |
| ----------------- | --------------------------- | ------------------------------------------------------------------------- | ------------------------------------ |
| `IDrawingContext` | `types/platform/context.ts` | 平台无关 2D 绘图 (~50 方法)                                               | `WebDrawingContext` (banvasgl-react) |
| `IDrawingSurface` | `types/platform/surface.ts` | 平台画布工厂 + 双缓冲 + 帧调度（合并旧 IPlatformCanvas + IEngineContext） | `WebPlatformCanvas` (banvasgl-react) |
...
@banyuan/banvasgl-react        # Web 平台注入 + React Hook
    ...
    ├── WebPlatformCanvas (HTMLCanvasElement → IDrawingSurface)
...
- IDrawingSurface 的 export / toBlob 为可选方法——不是所有平台都支持
- 引擎通过 `App.create(surface)` 工厂接收 IDrawingSurface 注入
- IDrawingSurface 承载帧调度原语（`requestFrame` / `cancelFrame`），取代旧 `IEngineContext.composite()` 双缓冲合成

**反例：**

- 方案 A：在 banvasgl 内部定义 WebSurface 实现，通过条件编译切换——条件编译增加构建复杂度，且无法在类型层面阻止非 Web 代码使用 DOM 类型
...
### A9. Rust 原生核心 — Step 1：2D 全量改写并验证功能

**未实施** · refines A8b（平台抽象层使原生核心迁移成为可能）

#### 两步走策略

BanvasGL 的 Rust 改写分为两个独立步骤，先后序严格：

| 步骤              | 内容                                          | 依赖              | 目标                           |
| ----------------- | --------------------------------------------- | ----------------- | ------------------------------ |
| **Step 1**（A9）  | TypeScript → Rust 全量 2D 移植 + 功能对等验证 | 仅需 A8b 平台抽象 | 功能与 TS 版一致，性能大幅提升 |
| **Step 2**（A9a） | 渲染管线从 CPU 软件绘制重构为 wgpu GPU 渲染   | Step 1 验证通过   | 2D+3D 统一渲染，充分利用 GPU   |

**Step 1 不与 Step 2 耦合。** Step 1 的交付物是一个功能完整的 Rust 2D 引擎核心——渲染仍走 CPU（tiny-skia / Canvas 2D 指令），但场景图遍历、布局计算、命中检测、FlowSchema 执行、动画插值等计算密集型操作全部在 Rust 中完成。

#### 为什么分两步

- **风险隔离**：全量 TS→Rust 移植本身就是大工程（Graph 17 种 + View 体系 + 5 种 Layout + Flow 25 种 NodeKind + Animation），再叠加 3D 管线重构会导致验证困难——出问题时无法判断是迁移 bug 还是渲染管线 bug
- **验证闭环**：Step 1 交付后，可以用现有 TS 版的所有测试用例做 A/B 对比（相同输入 → 相同像素输出 → 相同 Flow 执行路径），确保 100% 功能对等
- **商业闭环**：Step 1 完成即已获得 Rust 的性能收益（编译型 + 无 GC），可以作为独立的加速引擎发布，无需等待 Step 2

#### MVP 阶段策略

**MVP 阶段（商业落地前）不进行 Rust 改写。** 当前 TypeScript 引擎的性能足以支撑产品验证和早期商业化。Rust 改写是规模化的产物——在以下条件满足后再启动：

- 商业模型得到验证，有持续收入支撑团队扩张
- 客户场景出现 TS 引擎的性能瓶颈（大场景 1000+ 图形、移动端帧率不足等）
- 团队具备 Rust 工程能力或能招聘到对应人才

A8b 的平台抽象接口（IDrawingContext / IDrawingSurface）是天然的 FFI 边界。引擎核心的场景图遍历、布局计算、命中检测、FlowSchema 执行、动画插值等计算密集型操作可以从 TypeScript 迁移至 Rust，通过 WASM（Web）/ napi-rs（Node）/ UniFFI（移动端）暴露统一 C-ABI，不同平台注入各自的 IDrawingContext 实现。
...
| 2D 渲染后端   | tiny-skia (CPU) + vello (GPU) + cosmic-text (文本) + kurbo (几何) | Skia (旗舰但构建复杂)         |
...
| 阶段              | 时间   | 内容                                                                                                               |
| ----------------- | ------ | ------------------------------------------------------------------------------------------------------------------ |
| Phase 1：原型验证 | 1-2 月 | Rust workspace + wasm-bindgen；最小 Scene + Circle → render；A/B 对比 TS 版像素输出                                |
| Phase 2：核心迁移 | 3-4 月 | Graph (17 种) → View 体系 → Layout (flex/list/grid/scroll) → Flow 执行器 (25 种 NodeKind) → Camera/Animation       |
| Phase 3：包整合   | 2-3 月 | @banyuan/banvasgl-native npm 包；RustDrawingSurface 实现 IDrawingSurface；banvasgl-react 新增 useNativeEngine 切换 |
| Phase 4：多平台   | 2-3 月 | WASM (web) + napi-rs (Node) + UniFFI (iOS/Android)                                                                 |

**Step 1 的能力复用：** 当前 TypeScript 版本已建立的平台抽象（`IDrawingSurface` / `IDrawingContext`）和 Actions 接口（`IBanvasActions`）在 Step 1 中保持不变——Rust 核心仅替换引擎内部的纯计算逻辑（Graph/View/Layout/Flow/Animation），对外接口契约零改动。这意味着所有依赖 `@banyuan/banvasgl` 的上层代码（banvasgl-react、banyan 前端）无需任何修改即可受益于 Rust 加速。
...
### A9a. 3D 渲染管线重构 — Step 2：wgpu 驱动的 GPU 渲染

**未实施** · refines A9（Step 1 Rust 2D 核心验证通过后启动）

#### 定位

Step 2 是 **Step 1 的上层建筑**——在已验证的 Rust 2D 引擎核心之上，将渲染管线从 CPU 软件绘制（tiny-skia → Canvas 2D 指令）重构为 wgpu 驱动的 GPU 渲染。Step 2 不改动引擎核心的场景图 / View / Layout / Flow / Animation 逻辑——只替换渲染后端。

Step 1 的核心收益是「用编译型语言重写引擎计算逻辑」，Step 2 的核心收益是「打开 GPU 渲染通道」。Rust 生态提供了从 GPU 驱动抽象（wgpu）到声明式 2D 渲染器（vello）到 3D 场景图（rend3）的完整链。这套链路可以让 banvasgl 在不改上层 API（`IDrawingSurface` / `IDrawingContext`）的前提下，把渲染从 CPU Canvas 2D 升级到 GPU。
...
**决策链：** A8b（平台抽象）→ A9 Step 1（Rust 2D 全量改写）→ A9a Step 2（3D 管线重构）。Step 1 建立了功能对等的 Rust 2D 核心，Step 2 在此基础上将渲染后端从 CPU 切换为 wgpu GPU 管线，vello + rend3 作为渲染后端，tiny-skia 作为 CPU 降级方案。

**约束：**

- `IRenderBackend` 是引擎核心的内部 trait，不暴露给上层——上层仍通过 `IDrawingSurface` 注入 surface，引擎内部自动选择 GPU / CPU 后端
...
**实施时机：** Step 1 Phase 2（引擎核心迁移）完成后、Phase 3（包整合）之前。原型可以单独验证：在 `crates/banvasgl-render/` 中实现 `vello` 后端，用 Step 1 的 `Scene + Circle` 场景验证 GPU 渲染帧率对比。
