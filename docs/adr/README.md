...
| 粒度 | 文件 | 分组概述 |
|------|------|---------|
| 架构级 | [`architecture.md`](./engine/architecture.md) | 顶层组织（A1 六层架构）→ 渲染（A2→A2a）→ 交互（A3→A3a）→ 视图（A4→A4a）→ Flow（A5 内置子系统定义→A5a 物理隔离→A5b 执行器隔离）→ 序列化（A6→A6a）→ 物料（A7→A7a）→ 宿主集成（A8→A8a）→ 平台抽象（A8b）→ Rust 原生核心（A9）→ 统一 2D/3D GPU 渲染（A9a） |
