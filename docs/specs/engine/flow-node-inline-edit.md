...
export class ConditionDescriptor extends NodeKindDescriptor<'condition'> {
    ...
    derivePorts(schema): PortDescriptor[] {
        ...
        return [
            ...
            { id: `${id}_true`,  direction: 'output', semantic: 'branch', label: 'true',  branchLabel: 'true',  maxConnections: 1 },
            { id: `${id}_false`, direction: 'output', semantic: 'branch', label: 'false', branchLabel: 'false', maxConnections: 1 },
            { id: `${id}_error`, direction: 'output', semantic: 'branch', label: 'error', branchLabel: 'error', maxConnections: 1 },
            // 数据端口：condition.left 和 condition.right 可通过连线赋值
            { id: `${id}_left`,  direction: 'input',  semantic: 'dataIn', label: 'left',  anchorField: 'condition.left',  maxConnections: 1 },
            { id: `${id}_right`, direction: 'input',  semantic: 'dataIn', label: 'right', anchorField: 'condition.right', maxConnections: 1 },
        ]
    }

    getFormFields(schema): FormFieldDescriptor[] {
        return [
            { key: 'condition.left',  label: '左值', widget: 'flowValue', required: true, boundPort: `${schema.id}_left` },
            { key: 'condition.op',    label: '运算符', widget: 'select', required: true, options: [
                { value: '==', label: '等于 ==' },
                { value: '!=', label: '不等于 !=' },
                { value: '>',  label: '大于 >' },
                { value: '>=', label: '大于等于 >=' },
            ...
            { key: 'condition.right', label: '右值', widget: 'flowValue', required: true, boundPort: `${schema.id}_right` },
        ]
    }
}
```
...
export class SubFlowDescriptor extends NodeKindDescriptor<'subFlow'> {
    ...
    getTitle(schema): string { return schema.name || '子流程' }
...
export class HttpRequestDescriptor extends NodeKindDescriptor<'httpRequest'> {
    ...
    getTitle(): string { return 'HTTP 请求' }
    ...
    derivePorts(schema): PortDescriptor[] {
        ...
        return [
            ...
            { id: `${id}_in`,    direction: 'input',  semantic: 'controlFlow', maxConnections: Infinity },
            { id: `${id}_out`,   direction: 'output', semantic: 'controlFlow', maxConnections: 1 },
            { id: `${id}_error`, direction: 'output', semantic: 'branch', branchLabel: 'error', label: 'error', maxConnections: 1 },
            // 数据输入端口
            { id: `${id}_url`,   direction: 'input',  semantic: 'dataIn', label: 'url',  anchorField: 'url',  maxConnections: 1 },
            { id: `${id}_body`,  direction: 'input',  semantic: 'dataIn', label: 'body', anchorField: 'body', maxConnections: 1 },
            // 数据输出端口
            { id: `${id}_result`, direction: 'output', semantic: 'dataOut', label: 'response', anchorField: 'outputVariable', maxConnections: Infinity },
        ]
    }

    getFormFields(schema): FormFieldDescriptor[] {
        return [
            { key: 'method', label: '请求方法', widget: 'select', required: true, options: [
                { value: 'GET', label: 'GET' },
            ...
            { key: 'outputVariable', label: '结果变量名', widget: 'text', required: true, placeholder: '如 response' },
        ]
    }
}
```
...
export class VariableDescriptor extends NodeKindDescriptor<'variable'> {
    ...
    getTitle(): string { return 'View 变量' }
    ...
    derivePorts(schema): PortDescriptor[] {
        return [
            { id: `${schema.id}_out`, direction: 'output', semantic: 'dataOut', label: 'value', maxConnections: Infinity },
        ]
    }
...
| subFlow | rect | subroutine | _in, _out | _param_*(in) × N, _result_*(out) × N | — |
