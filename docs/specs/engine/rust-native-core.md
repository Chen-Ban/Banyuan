...
2025-06-21 审计发现 **17 处 DOM/平台耦合违规**（集中于 `src/graph/media/`、`src/foundation/style/`、`src/engine/App.ts`）。这些违规是本次 Rust 重构要根除的目标。

### 0.1 渲染循环 — `requestAnimationFrame`

| 文件 | 行号 | 代码 | 根因 |
|------|------|------|------|
| `engine/App.ts` | 556 | `requestAnimationFrame((timestamp) => ...)` | 帧调度硬编码浏览器 API |
| `engine/App.ts` | 490, 503 | `cancelAnimationFrame(...)` | 同上 |

**Rust 侧方案**：帧循环由 Host 拥有（rAF / setInterval / 线程 loop），Host 每帧调 `engine.render_frame(timestamp)`。Rust 不感知调度机制，只负责帧内计算。

### 0.2 DOM 元素创建 — `document.createElement`

| 文件 | 行号 | 代码 |
|------|------|------|
| `graph/media/ImageElement.ts` | 236 | `document.createElement('canvas')` |
| `graph/media/VideoElement.ts` | 110 | `document.createElement("video")` |
| `graph/media/VideoElement.ts` | 424 | `document.createElement("canvas")` |
| `foundation/style/Video.ts` | 269 | `document.createElement('video')` |

**根因**：`getImageData()` 需要临时 canvas 提取像素；`loadVideo()` 需要 `<video>` 解码。
**Rust 侧方案**：像素操作走 `DrawingContext.create_image_data()/get_image_data()`；Host 负责解码（Web: `<video>` + `createImageBitmap`，Native: ffmpeg），每帧提取为 `PixelBuffer` 传给 Rust 存储。Rust 只持有解码后的像素数据，不持有解码器。

### 0.3 浏览器构造函数 — `new Image()`

| 文件 | 行号 | 代码 |
|------|------|------|
| `graph/media/ImageElement.ts` | 108 | `const img = new Image()` |
| `foundation/style/Image.ts` | 166 | `new globalThis.Image()` |

**根因**：图片加载直接依赖浏览器 `HTMLImageElement` 的网络解码能力。
**Rust 侧方案**：Host 负责网络获取 + 平台解码（Web: `fetch` + `createImageBitmap` → ImageData，Native: `image` crate），生成 RGBA8 像素后构造 `PixelBuffer`（Rust concrete struct）传入引擎。`PixelBuffer` 是纯数据 struct，非 trait。Rust 用它在布局/命中检测中查询宽高、提取像素，渲染时传回 `DrawingContext::draw_image()`。

### 0.4 DOM 类型泄露

| 文件 | 行号 | 泄露的类型 |
|------|------|-----------|
| `graph/media/VideoElement.ts` | 42 | `HTMLVideoElement` 公共属性 |
| `foundation/style/Video.ts` | 265 | `HTMLVideoElement` 返回类型 |
| `graph/media/ImageElement.ts` | 390 | `HTMLCanvasElement` 参数（@deprecated） |
| `graph/media/MediaElement.ts` | 304 | `ImageData` 抽象返回类型 |
| `graph/media/ImageElement.ts` | 233 | `ImageData` 返回类型 |
...
| 文件 | 行号 | 内容 | 严重度 |
|------|------|------|--------|
| `view/View/View.ts` | 81 | `localStorage.getItem()` — debug 特性 | 🟡 低 |
| `engine/renderer/Renderer.ts` | 100 | `performance.now()` | 🟡 低 |
...
│   │           └── canvas.rs          # PlatformCanvas trait
│   │
...
│   │   │   └── WebPlatformCanvas.ts   # → Rust PlatformCanvas trait
│   │   └── hooks/
...
| `graph/analytic/*` (9 种) | `graph/analytic/` — 纯几何计算 | 2 |
| `graph/combined/*` (6 种) | `graph/combined/` | 2 |
...
| `view/addon/*` (5 addons) | `view/addon/` | 2 |
...
| `engine/material/*` | `material/` — 物料系统 | 3 |
...
| `engine/serialization/Serializer.ts` | 序列化/反序列化 — 走 JSON ↔ Rust 结构体转换，TS 侧处理类型注册 |
...
| 模块 | 原因 |
|------|------|
| `graph/media/ImageElement.ts` (DOM 部分) | `new Image()` / `document.createElement` 替换为 `PixelBuffer` |
| `graph/media/VideoElement.ts` (DOM 部分) | `document.createElement('video')` 替换为 Host 解码 → `PixelBuffer` |
| `foundation/style/Image.ts` (DOM 部分) | `new globalThis.Image()` 替换为 Host 注入 |
| `foundation/style/Video.ts` (DOM 部分) | `document.createElement('video')` 替换为 Host 注入 |
| `view/View/View.ts` L81 localStorage | 替换为 `DebugOptions` struct 由宿主注入 |
...
```rust
// crates/banvasgl-core/src/platform/canvas.rs

pub trait PlatformCanvas {
    fn main_ctx(&mut self) -> &mut dyn DrawingContext;
    fn buffer_ctx(&mut self) -> &mut dyn DrawingContext;
...
impl Engine {
    pub fn new(platform: Box<dyn PlatformCanvas>, options: EngineOptions) -> Self;
...
**交付物**：
- Cargo workspace + `banvasgl-core` crate（math / graph / simple scene / DrawingContext trait / PlatformCanvas trait）
- `banvasgl-native` npm 包（wasm-pack 产出）
- 性能对比报告（TS vs Rust WASM：场景创建、渲染帧时、内存）

**不在此 Phase**：View 树、布局、Flow、动画、事务。

> Phase 1 的 `banvasgl-react` 不需要新增适配器——`WebDrawingContext`（已有）实现 DrawingContext trait，`WebPlatformCanvas`（已有）实现 PlatformCanvas trait。Host 侧 rAF loop 直接调 `engine.render_frame(timestamp)`，无需额外抽象。
...
| `crates/banvasgl-core/src/platform/canvas.rs` | PlatformCanvas trait |
...
**目标**：Graph（14 种） → View 体系 → 布局引擎 → Camera → Renderer → Scene → Style。

| 子阶段 | 内容 | Rust 模块 |
|--------|------|-----------|
| 2a: Graph | 14 种图形基元 + Bounds + 碰撞检测 | `graph/` |
...
| 审计项 | TS 违规位置 | Rust 侧根方案 | Phase |
|--------|------------|--------------|-------|
| `requestAnimationFrame` | `engine/App.ts:556` | Host 拥有帧循环，Rust 暴露 `render_frame(timestamp)` | 1 |
| `cancelAnimationFrame` | `engine/App.ts:490,503` | Host 管理 rAF handle，Rust 不感知 | 1 |
| `document.createElement('canvas')` | `graph/media/ImageElement.ts:236` | `DrawingContext::create_image_data()` | 2 |
| `document.createElement('canvas')` | `graph/media/VideoElement.ts:424` | 同上 | 2 |
| `document.createElement('video')` | `graph/media/VideoElement.ts:110` | Host 解码为 `PixelBuffer`，`engine.update_video_frame()` | 2 |
| `document.createElement('video')` | `foundation/style/Video.ts:269` | 同上 | 2 |
| `new Image()` | `graph/media/ImageElement.ts:108` | Host 解码为 `PixelBuffer`，`engine.add_image()` | 2 |
| `new globalThis.Image()` | `foundation/style/Image.ts:166` | 同上 | 2 |
| `localStorage.getItem()` | `view/View/View.ts:81` | `DebugOptions` struct 由 `Engine::create(opts)` 传入 | 2 |
| `HTMLVideoElement` 类型 | `graph/media/VideoElement.ts:42` | 废弃 — Rust 只持有 `VideoId` + `PixelBuffer` | 2 |
| `HTMLVideoElement` 返回类型 | `foundation/style/Video.ts:265` | 废弃 — 返回 `PixelBuffer` | 2 |
| `HTMLCanvasElement` 参数 | `graph/media/ImageElement.ts:390` | 已 @deprecated — 不迁移 | - |
| `CanvasImageSource` cast | `graph/media/ImageElement.ts:244` | 废弃 — 使用 `PixelBuffer` | 2 |
| `ImageData` 返回类型 | `graph/media/MediaElement.ts:304` | `PixelBuffer` struct（Rust 自有） | 2 |
| `performance.now()` | `engine/renderer/Renderer.ts:100` | Host 传入的 timestamp 参数 / `std::time::Instant` | 2 |
...
| `@banyuan/banvasgl-react` | 无需新增适配器 — 已有 `WebDrawingContext` / `WebPlatformCanvas` 直接对应 2 个 trait；hook 层增加 rAF loop 调 `engine.render_frame()` |
