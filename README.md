...
**平台无关**：核心零 React、零 DOM 依赖，通过 `IDrawingContext` / `IDrawingSurface` 两个平台抽象接口实现跨平台。同一套引擎代码可运行在 Web、iOS、Android、Node.js 等多种平台。
...
BanvasGL 的 Web 平台适配层。提供 `createWebDrawingContext`（CanvasRenderingContext2D → IDrawingContext）、`WebSurface`（HTMLCanvasElement → IDrawingSurface，含双缓冲合成）两个 Web 平台注入，以及 `useFixedCanvasInit` / `useAdaptiveCanvasInit` / `useCanvasCamera` / `screenToWorld` / `worldToScreen` / `getCameraZoomLevel` 等 React Hook 和坐标转换工具。`@banyuan/banvasgl` 与 `react` 均为其 peerDependency。
...
运行在租户 ECS 上的部署代理。通过 WebSocket 与 Banyan 后端建立长连接，接收部署指令后在租户机器上脚手架化项目与服务器代码（`scaffoldProject` / `scaffoldServer`），执行静态站点（`deployStatic`）或全栈应用（`deployFullstack`）部署，完成 Docker 构建与 Nginx 配置。其内置的 flowRunner 模块通过 `createServerFlowRunner()` from `@banyuan/banvasgl/flow/server` 执行云函数的 FlowSchema——Banyan 后端只负责存储 FlowSchema 定义，真正的执行宿主在用户 ECS 产物中。提供 CLI 入口（`deploy-agent`）。
...
**渲染层与宿主解耦**：通过 `IDrawingContext` / `IDrawingSurface` 平台抽象接口，Canvas 2D 在浏览器、Electron、未来的移动端行为一致，自研引擎换来跨平台路径清晰。
