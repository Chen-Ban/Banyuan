...
引擎采用单入口设计（`src/index.ts`），仅导出核心图形能力。编辑态 Hook（`useDesignBanvas`）、运行态 Hook（`useRuntimeBanvas`）、流程图编辑器（`useFlowBanvas`）已拆分为独立包，通过 `@banyuan/banyan-sdk` 统一消费。
...
一个低代码平台的运行态产物不应包含编辑器代码——编辑器有 Worker 管理、交互分发、组件物料等重逻辑，全塞进运行态 bundle 会让产物体积膨胀数倍。通过将 BanvasGL 拆为核心引擎（`@banyuan/banvasgl`）+ 编辑态绑定（`@banyuan/banvas-design`）+ 运行态绑定（`@banyuan/banvas-runtime`）+ 流程图编辑器（`@banyuan/flow-design`），每个消费方只引入自己需要的部分，构建时自动 tree-shake。
...
│   ├── BanvasDesign/       # 编辑态 React Hook (@banyuan/banvas-design)
...
│   ├── BanyanSDK/          # 统一 SDK 聚合导出 (@banyuan/banyan-sdk)
│   └── XiangDi/            # AI Agent 引擎 (@banyuan/xiangdi-agent)
├── apps/
│   ├── banyan/             # 低代码平台
...
@banyuan/flow (独立，无 workspace 依赖)
    ↑
@banyuan/banvasgl (依赖 @banyuan/flow)
    ↑
@banyuan/banvas-runtime (peerDep: canvas)
@banyuan/banvas-design (peerDep: canvas + canvas-runtime)
@banyuan/flow-design (peerDep: canvas + canvas-runtime)
    ↑
@banyuan/banyan-sdk (聚合以上全部)
@banyuan/xiangdi-agent (optional peerDep: canvas)
    ↑
