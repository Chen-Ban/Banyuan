...
│   ├── banvasgl-react/      # BanvasGL Web 平台注入 + React Hook (npm: @banyuan/banvasgl-react)
│   ├── banvas-react-runtime/ # BanvasGL 运行策略层：高级交互识别 + useRuntimeBanvas hook (npm: @banyuan/banvas-react-runtime)
...
apps/banyan/frontend ──▶ @banyuan/banvasgl (workspace:*)
                    ──▶ @banyuan/banvasgl-react (workspace:*)
...
@banyuan/banvasgl-react ──peerDep──▶ @banyuan/banvasgl + react   # Web 平台注入 + React Hook

@banyuan/banvas-react-runtime ──▶ @banyuan/banvasgl-react (workspace:*)  # 复用 useCanvasInit 底座
                               ──peerDep──▶ @banyuan/banvasgl + react   # 用户 ECS 产物运行态
...
banvasgl 与 banvasgl-react / banvas-react-runtime 的分工：banvasgl 是「运行时」，只提供机制（原子事件 / 几何变换 / FlowSchema 执行）和 `subscribe/getVersion/notify` 状态订阅原语，零 DOM / 零 React 依赖；banvasgl-react 是「Web 平台注入层」，封装 `useCanvasInit`（画布初始化）/ `useCanvasCamera`（相机交互）/ `WebDrawingContext`（Web 平台适配器）；banvas-react-runtime 是「运行策略层」，封装高级交互识别（ClickRecognizer / DragRecognizer）和运行态 `useRuntimeBanvas` hook（复用 banvasgl-react 的 `useCanvasInit` 底座），被打包进用户 ECS 产物，将策略注入 banvasgl 运行时（详见 docs/adr/engine/architecture.md A0 机制/策略分离原则）。

### 关键设计决策：Flow 融合进 BanvasGL

View 是带有流程控制语义的对象——每个 View 通过 `events`（onClick 等 12 个事件）和 `lifetimes`（onCreated/onAttach/onDestroy）字段绑定 FlowSchema。Scene 同样有生命周期（onLoad/onUnload/onShow/onHide）绑定 FlowSchema。渲染层和流程控制天然耦合，因此 BanvasGL 定位为「面向声明式 UI 的 2D 图形运行时（含流程控制）」而非纯渲染包。作为运行时，它只提供机制（原子事件 / 几何变换 / FlowSchema 执行），高层交互策略由上层注入（详见 docs/adr/engine/architecture.md A0）。

Flow 不再是独立 npm 包，而是 `@banyuan/banvasgl` 的内部子模块（源码位于 `src/foundation/flow/`），通过子路径导出实现物理隔离：后端 `import '@banyuan/banvasgl/flow/server'` 只加载流程执行器，不会引入图形引擎代码（tsup splitting + package.json exports）。App 运行态通过 `createClientFlowRunner()` 创建 FlowRunner 实例，Scene.triggerSchema 直接调用 `flowRunner.run(schema)`。
...
- 六层架构：`engine`（App/Scene/Renderer/Camera/TransactionManager/material/serialization 等）/ `view`（View 基类及子类）/ `graph`（图形基元）/ `foundation`（数学/样式/动画/常量/工具 + Flow 运行时，仅依赖 types）/ `types`（纯接口契约，零实现）/ `actions`（封装的高阶操作函数：appActions/pageActions/viewActions）。React 宿主集成（useCanvasInit 等 hook + Web 平台适配器）在独立包 `@banyuan/banvasgl-react` 中。Flow 类型在 `src/types/foundation/flow/`，实现在 `src/foundation/flow/`
...
- View 是带有流程控制语义的对象：`events`（12 个事件处理器）+ `lifetimes`（onCreated/onAttach/onDestroy 三个生命周期钩子），类型均为 `FlowSchema | null`
...
- App 持有 `FlowRunner` 实例（`createClientFlowRunner()`），Scene.triggerSchema 直接调用 `flowRunner.run(schema)`，无需外部注入
- React 宿主集成在独立包 `@banyuan/banvasgl-react`（`packages/banvasgl-react/src/`），导出 `useCanvasInit`（画布初始化）、`useCanvasCamera`（相机控制）、`WebDrawingContext` / `WebSurface`（Web 平台适配器）。banvasgl 核心零 DOM / 零 React 依赖

### BanvasGL Flow 子模块（@banyuan/banvasgl/flow）

- 声明式 FlowSchema 执行器，节点图（nodes + entry）驱动，Push-Pull 混合调度
- 作为 banvasgl 内部子模块存在于 `src/foundation/flow/`，通过 package.json exports 子路径导出
- v2.0.0 将边消解为节点内嵌引用：控制流由 `slots[*].next` 字段承载，数据依赖由 `SlotValue = unknown | DataRef` 承载
- v2.1.0 统一执行器模型：所有 25 种 NodeKind 通过 `ExecutorRegistry`（按 NodeKind 索引的映射类型）分发，Runner 退化为纯编排外壳
- 前后端执行器分离：`@banyuan/banvasgl/flow/client`（前端：setViewData/setViewVisible/playAnimation/navigate/cloudFunction + 共享）和 `@banyuan/banvasgl/flow/server`（后端：httpRequest/dbQuery/dbInsert/dbUpdate/dbDelete + 共享）
- 共享执行器：source（literal/context）、compute（math/compare/logic/concat/format/get）、control（condition/loop/parallel/return）、function、setVariable
- 后端使用 `createServerFlowRunner()`，通过 `BackendCapProxy` 注入 db 和 httpClient 能力
- 子路径导出：`./flow/client`（前端预设工厂）、`./flow/server`（后端预设工厂）；Flow 类型从主入口 `.` 导出
- 包级 exports 仅有三个子路径：`.`（主入口）、`./flow/client`、`./flow/server`；React Hook 在独立包 `@banyuan/banvasgl-react` 中。**不存在 `./flow` 公开子路径**——内部组件（FlowRunner、NodeExecutor 注册表、各求值器）不对外暴露，只暴露预组装工厂
...
| BanvasGL React Hook 层（useCanvasInit/useCanvasCamera） | `packages/banvasgl-react/src/hooks/index.ts`                        |
| banvas-react-runtime 运行策略层入口                     | `packages/banvas-react-runtime/src/index.ts`                        |
| banvas-react-runtime 运行态 Hook                        | `packages/banvas-react-runtime/src/hook/useRuntimeBanvas.tsx`       |
| banvas-react-runtime 交互识别器                         | `packages/banvas-react-runtime/src/interaction/`                    |
| Flow 子模块入口                                         | `packages/banvasgl/src/foundation/flow/index.ts`                    |
| Flow 客户端预设                                         | `packages/banvasgl/src/foundation/flow/presets/client.ts`           |
| Flow 服务端预设                                         | `packages/banvasgl/src/foundation/flow/presets/server.ts`           |
| Flow 类型定义                                           | `packages/banvasgl/src/types/foundation/flow/schema.ts`             |
| Flow 执行器注册表类型                                   | `packages/banvasgl/src/types/foundation/flow/executor.ts`           |
...
- **禁止**在 `packages/` 目录下的库包中引入 React/DOM 等宿主环境依赖（`@banyuan/banvasgl-react` 除外，该包专门承载 React + Web 绑定）
...
- 运行态高级交互（点击/拖拽识别）属于 `@banyuan/banvas-react-runtime`，编辑态机制属于 `@banyuan/banvasgl`，新增交互能力时注意区分归属（机制 vs 策略）
...
- [banvasgl-react README](./packages/banvasgl-react/README.md) — BanvasGL Web 平台注入 + React Hook
- [banvas-react-runtime README](./packages/banvas-react-runtime/README.md) — BanvasGL 运行策略层（高级交互识别 + 运行态 Hook）
